...
namespace io {
HikCamera::HikCamera(double exposure_ms, double gain, bool autocap)
...
HikCamera::~HikCamera() {
  if (this->guard.protectthread.joinable())
    this->guard.protectthread.join();
...
void HikCamera::capture_init() {
  ...
  if (!this->parame.autocap) {
    ...
    if (MV_OK != ret) {
      tools::logger()->warn(
          "Set Acquisition Mode to Continuous fail! nRet [0x%x]\n", ret);
      return;
    }
    ...
    if (MV_OK != ret) {
      tools::logger()->warn(
          "Set Trigger Activation to RisingEdge fail! nRet [0x%x]\n", ret);
      return;
    }
  } else {
    //将触发模式设置为开启 (On)
  ...
  if (ret != MV_OK) {
    tools::logger()->warn("MV_CC_SetFloatValue(set framerate) failed: {:#x}",
                          ret);
    return;
  }
...
void HikCamera::ProtectRunning() {
  this->guard.protectthread = std::thread{[this]() -> void {
    std::unique_lock<std::mutex> lock(this->guard.mux);
    while (true) {
      this->guard.HikIsquit.wait(
          lock, [this] { return (this->HikState == Hik::Stopped); });
      this->capture_stop();
      this->capture_init();
      if (conCapOpen)
        this->continueCap(this->MaxframeNum);
    }
  }};
}
