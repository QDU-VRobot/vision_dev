...
void CommandLine::addOption(const std::string& shortName, const std::string& longName,
                            ...
                            const std::string& defaultValue)
{
  ...
  if (!longName.empty()) longOptions_[longName] = shortName;
}
...
bool CommandLine::parse(int argc, char* argv[])
{
  ...
  for (int i = 1; i < argc; ++i)
  {
    ...
    if (arg.rfind("-", 0) == 0)
    {
      ...
      {
        key = arg.substr(2);
        if (longOptions_.count(key)) shortKey = longOptions_[key];
      }
      // 短选项
      else
      {
...
{
  if (!shortOptions_.count(shortName)) return "";
...
{
  if (!shortOptions_.count(shortName)) return false;
...
void CommandLine::printHelp(const std::string& progName) const
{
  ...
  for (const auto& [shortName, opt] : shortOptions_)
  {
    ...
    line << "  " << shortName;
    if (!opt.longName.empty()) line << ", --" << opt.longName;
    if (opt.requiresValue) line << " <value>";
    std::cout << line.str() << "\n      " << opt.description;
    if (!opt.defaultValue.empty()) std::cout << " (default: " << opt.defaultValue << ")";
    std::cout << "\n";
  }
}
