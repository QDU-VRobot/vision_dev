...
ParamCanvas::ParamCanvas(const std::string& name, int width, int height, int max_points)
    : winname_(name),
      ...
      window_init_flag_(false),
      smooth_alpha_(0.1f),
      smooth_min_y_(0.0f),
      smooth_max_y_(1.0f),
      img_(height, width, CV_8UC3, cv::Scalar(0, 0, 0)),
      colors_{cv::Scalar(0, 0, 255),    cv::Scalar(0, 255, 0),
              cv::Scalar(255, 0, 0),    cv::Scalar(255, 255, 0),
              cv::Scalar(255, 0, 255),  cv::Scalar(0, 255, 255),
              cv::Scalar(100, 200, 50), cv::Scalar(200, 100, 50),
              cv::Scalar(50, 100, 200), cv::Scalar(100, 50, 200)}
{
}
...
void ParamCanvas::setMaxPoints(int max_points) { max_points_per_param_ = max_points; }
...
void ParamCanvas::addParam(const std::string& key, float value)
{
  ...
  if (dq.size() >= max_points_per_param_) dq.pop_front();
  dq.push_back(value);
}

void ParamCanvas::addParam(const std::string& key, const std::vector<float>& values,
                           bool overwrite)
{
  auto& dq = param_map_[key];
  if (overwrite)
  ...
  // 计算需要新加入的元素数量
  size_t new_add_num =
      std::min(values.size(), static_cast<size_t>(max_points_per_param_));
  // 计算需要删除的元素数量
  ...
  dq.insert(dq.end(),
            values.begin() +
                std::max(0, static_cast<int>(values.size()) - max_points_per_param_),
            values.end());
...
void ParamCanvas::reset() { param_map_.clear(); }
...
void ParamCanvas::alignParams()
{
  ...
  {
    while (dq.size() < max_size) addParam(key, dq.back());
    // addParam(key,-100);
  }
}
...
{
  cv::line(img, {margin_left_, margin_top_}, {margin_left_, img_row_ - margin_bottom_},
           {200, 200, 200});
  cv::line(img, {margin_left_, img_row_ - margin_bottom_},
           {img_col_ - margin_right_, img_row_ - margin_bottom_}, {200, 200, 200});
  ...
  for (int i = 0; i <= num_y_ticks_; ++i)
  {
    ...
    cv::putText(img, cv::format("%.2f", val), {5, y + 5}, cv::FONT_HERSHEY_SIMPLEX, 0.4,
                {200, 200, 200});
  }
  ...
  for (int i = 0; i <= num_x_ticks_; ++i)
  {
    ...
    cv::line(img, {x, img_row_ - margin_bottom_ - 5}, {x, img_row_ - margin_bottom_ + 5},
             {200, 200, 200});
    cv::putText(img, std::to_string(x_val), {x - 10, img_row_ - margin_bottom_ + 20},
                cv::FONT_HERSHEY_SIMPLEX, 0.4, {200, 200, 200});
  }
}

void ParamCanvas::drawParams(cv::Mat& img, float min_y, float max_y)
{
  if (param_map_.empty()) return;

  auto mapX = [&](int i)
  {
    return margin_left_ + int(float(i) / (max_points_per_param_ - 1) *
                              (img_col_ - margin_left_ - margin_right_));
  };
  auto mapY = [&](float y)
  {
    return margin_top_ +
           int((max_y - y) / (max_y - min_y) * (img_row_ - margin_top_ - margin_bottom_));
  };
  ...
  for (const auto& [key, dq] : param_map_)
  {
    ...
    // 在图像的左上角显示参数名称
    cv::putText(img, key, {margin_left_ + 5, margin_top_ + 20 + idx * 15},
...
void ParamCanvas::show()
{
  ...
  do
  {
    ...
    {
      int x = margin_left_ + int(float(x_idx) / (max_points_per_param_ - 1) *
                                 (img_col_ - margin_left_ - margin_right_));
      cv::line(img_, {x, margin_top_}, {x, img_row_ - margin_bottom_}, color, 1,
               cv::LINE_AA);
      if (!label.empty())
      {
        cv::putText(img_, label, {x + 3, margin_top_ + 15}, cv::FONT_HERSHEY_SIMPLEX, 0.4,
                    color, 1);
      }
    }
    ...
    {
      int y =
          margin_top_ + int((smooth_max_y_ - y_val) / (smooth_max_y_ - smooth_min_y_) *
                            (img_row_ - margin_top_ - margin_bottom_));
      cv::line(img_, {margin_left_, y}, {img_col_ - margin_right_, y}, color, 1,
               cv::LINE_AA);
      if (!label.empty())
      {
        cv::putText(img_, label, {margin_left_ + 5, y - 3}, cv::FONT_HERSHEY_SIMPLEX, 0.4,
                    color, 1);
      }
    }
  } while (0);
