...
WindowAutoLayout::Ptr WindowAutoLayout::get()
{
  ...
  {
    auto result =
        std::make_shared<WindowAutoLayout>(cv::Size(1920, 1080), cv::Size(320, 240));
...
{
  if (hasWindow(name)) return;  // 已存在则忽略
  Node n;
  n.name = name;
  n.desired = desired;
...
void WindowAutoLayout::relayout() { relayout_(); }
...
void WindowAutoLayout::relayout_()
{
  ...
  for (int cols = 1; cols <= N; ++cols)
  {
    ...
    if (cellW < minWin_.width || cellH < minWin_.height) continue;  // 不满足最小阈值
...
void WindowAutoLayout::applyRect_(Node& n, const cv::Rect& target)
{
  ...
  if (animate_)
  {
    ...
    for (int s = 1; s <= animSteps_; ++s)
    {
      ...
      int h =
          static_cast<int>(std::lround(from.height + t * (target.height - from.height)));
