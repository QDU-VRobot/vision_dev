 ...
 */
inline std::string generateYmlPath(const std::string& source_file_path,
  ...
  (                                                                                 \
      ...
      {                                                                             \
        ...
        {                                                                           \
          ...
          if (paramStart == std::string_view::npos) return std::string("unknown");  \
          size_t funcNameEnd = prettyFunction.rfind("::", paramStart);              \
          if (funcNameEnd == std::string_view::npos) return std::string("unknown"); \
          size_t classNameStart = prettyFunction.rfind(' ', funcNameEnd) + 1;       \
          return std::string(                                                       \
              prettyFunction.substr(classNameStart, funcNameEnd - classNameStart)); \
        }();                                                                        \
        return result;                                                              \
      })()
...
// In ROS2 deployment we usually do NOT want libraries to auto-create/write config files
// at runtime. Define VC_DISABLE_AUTO_YML=1 to disable automatic load/write during static
// initialization.
#if defined(VC_DISABLE_AUTO_YML) && (VC_DISABLE_AUTO_YML == 1)
#define YML_MANAGER_CONSTRUCTOR_INIT(ParamStruct) \
  ParamStruct() { file_path = ""; }
#else
#define YML_MANAGER_CONSTRUCTOR_INIT(ParamStruct) \
  ParamStruct()                                   \
  {                                               \
...
#define YML_INIT(ParamStruct, add_param_function...)                             \
 public:                                                                         \
  ...
  std::string file_path;                                                         \
                                                                                 \
 public:                                                                         \
  void load(const cv::String& file_path,                                         \
            YmlType type = static_cast<YmlType>(YmlType::READ | YmlType::WRITE)) \
  {                                                                              \
    if (file_path.empty())                                                       \
    {                                                                            \
...
template <>
inline void readParam<size_t>(const cv::FileStorage& fs, const cv::String& name,
                              size_t& param)
{
  int temp = 0;
...
template <>
inline void writeParam<size_t>(cv::FileStorage& fs, const cv::String& name,
                               const size_t& param)
{
