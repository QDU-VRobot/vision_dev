...
{
 public:
  ParamCanvas(const std::string& name, int width = 800, int height = 600,
              int max_points = 300);
   ...
   */
  void addParam(const std::string& key, const std::vector<float>& values,
                bool overwrite = false);
  void markX(int x_index, const std::string& label = "",
             const cv::Scalar& color = {255, 255, 255});
  void markY(float y_value, const std::string& label = "",
             const cv::Scalar& color = {255, 255, 255});
  void clearMarks();
  void reset();
  void show();
