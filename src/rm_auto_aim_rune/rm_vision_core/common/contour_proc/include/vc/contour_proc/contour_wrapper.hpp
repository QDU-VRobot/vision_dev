 ...
 */
template <typename T>
concept ContourWrapperBaseType = std::is_same_v<std::remove_cv_t<T>, int> ||
...
class ContourWrapper
{
 public:
   ...
   */
  using KeyType = std::conditional_t<std::is_same_v<std::remove_cv_t<ValueType>, int>,
                                     float, ValueType>;
 ...
 public:
   ...
   * @param contour 轮廓点集
   */
  inline static std::shared_ptr<ContourWrapper<ValueType>> make_contour(
      const std::vector<PointType>& contour);
   ...
   * @brief 获取轮廓的凸包轮廓——接口
   */
  static std::shared_ptr<ContourWrapper<ValueType>> getConvexHull(
      const std::vector<std::shared_ptr<const ContourWrapper<ValueType>>>& contours);

 public:
  //--------------------【迭代器访问----------------------】
  using const_iterator = typename std::vector<PointType>::const_iterator;  // 常量迭代器
  using const_reverse_iterator =
      typename std::vector<PointType>::const_reverse_iterator;  // 常量反向迭代器
 ...
 private:
   ...
   * @brief 获取多轮廓的凸包轮廓
   */
  static std::shared_ptr<ContourWrapper<ValueType>> getConvexHullImpl(
      const std::vector<std::shared_ptr<const ContourWrapper<ValueType>>>& contours);
};
...
/**
 * @brief 类型检验，检验是否为
 * std::shared_ptr<ContourWrapper<T>>，T符合ContourWrapperBaseType
 */
template <typename T>
concept ContourWrapperPtrType = requires {
  requires std::is_same_v<std::shared_ptr<typename std::remove_cvref_t<T>::element_type>,
...
inline auto convert(const ContourPtr& contour)
...
// 创建轮廓对象
template <ContourWrapperBaseType _Tp>
inline std::shared_ptr<ContourWrapper<_Tp>> ContourWrapper<_Tp>::make_contour(
    const std::vector<PointType>& contour)
{
...
// 获取轮廓点集
template <ContourWrapperBaseType _Tp>
const std::vector<typename ContourWrapper<_Tp>::PointType>& ContourWrapper<_Tp>::points()
    const
{
  return __points;
}
...
cv::Rect ContourWrapper<_Tp>::boundingRect() const
{
  ...
  {
    const auto& contour = this->points();
    __bounding_rect = std::make_unique<cv::Rect>(cv::boundingRect(contour));
...
cv::RotatedRect ContourWrapper<_Tp>::minAreaRect() const
{
  ...
  {
    const auto& contour = this->points();
    __min_area_rect = std::make_unique<cv::RotatedRect>(cv::minAreaRect(contour));
...
// 获取轮廓的凸包
template <ContourWrapperBaseType _Tp>
const std::vector<typename ContourWrapper<_Tp>::PointType>&
ContourWrapper<_Tp>::convexHull() const
{
...
// 获取轮廓的凸包轮廓——接口
template <ContourWrapperBaseType _Tp>
std::shared_ptr<ContourWrapper<_Tp>> ContourWrapper<_Tp>::getConvexHull(
    const std::vector<std::shared_ptr<const ContourWrapper<_Tp>>>& contours)
{
  // 检查所有轮廓是否有效
...
// 迭代器访问实现
template <ContourWrapperBaseType _Tp>
typename ContourWrapper<_Tp>::const_iterator ContourWrapper<_Tp>::begin() const noexcept
{
  return __points.begin();
}

template <ContourWrapperBaseType _Tp>
typename ContourWrapper<_Tp>::const_iterator ContourWrapper<_Tp>::end() const noexcept
{
  return __points.end();
}

template <ContourWrapperBaseType _Tp>
typename ContourWrapper<_Tp>::const_iterator ContourWrapper<_Tp>::cbegin() const noexcept
{
  return __points.cbegin();
}

template <ContourWrapperBaseType _Tp>
typename ContourWrapper<_Tp>::const_iterator ContourWrapper<_Tp>::cend() const noexcept
{
  return __points.cend();
}

template <ContourWrapperBaseType _Tp>
typename ContourWrapper<_Tp>::const_reverse_iterator ContourWrapper<_Tp>::rbegin()
    const noexcept
{
  return __points.rbegin();
}

template <ContourWrapperBaseType _Tp>
typename ContourWrapper<_Tp>::const_reverse_iterator ContourWrapper<_Tp>::rend()
    const noexcept
{
  return __points.rend();
}

template <ContourWrapperBaseType _Tp>
typename ContourWrapper<_Tp>::const_reverse_iterator ContourWrapper<_Tp>::crbegin()
    const noexcept
{
  return __points.crbegin();
}

template <ContourWrapperBaseType _Tp>
typename ContourWrapper<_Tp>::const_reverse_iterator ContourWrapper<_Tp>::crend()
    const noexcept
{
  return __points.crend();
}

// STL接口实现
template <ContourWrapperBaseType _Tp>
size_t ContourWrapper<_Tp>::size() const noexcept
{
  return __points.size();
}

template <ContourWrapperBaseType _Tp>
bool ContourWrapper<_Tp>::empty() const noexcept
{
  return __points.empty();
}

template <ContourWrapperBaseType _Tp>
const typename ContourWrapper<_Tp>::PointType& ContourWrapper<_Tp>::operator[](
...
template <ContourWrapperBaseType _Tp>
const typename ContourWrapper<_Tp>::PointType& ContourWrapper<_Tp>::front() const noexcept
{
  return __points.front();
}

template <ContourWrapperBaseType _Tp>
const typename ContourWrapper<_Tp>::PointType& ContourWrapper<_Tp>::back() const noexcept
{
  return __points.back();
}

// 获取多轮廓的凸包轮廓
template <ContourWrapperBaseType _Tp>
std::shared_ptr<ContourWrapper<_Tp>> ContourWrapper<_Tp>::getConvexHullImpl(
    const std::vector<std::shared_ptr<const ContourWrapper<_Tp>>>& contours)
{
  size_t total_point_size = 0;
  for (const auto& contour : contours) total_point_size += contour->points().size();
