 ...
 */
inline void findContours(cv::InputArray image, std::vector<Contour_cptr>& contours,
...
inline void findContours(
    cv::InputArray image, std::vector<Contour_cptr>& contours,
    std::unordered_map<Contour_cptr, std::tuple<Contour_cptr, Contour_cptr, Contour_cptr,
                                                Contour_cptr>>& hierarchy,
...
void drawContour(cv::Mat& image, const std::shared_ptr<const ContourWrapper<T>>& contour,
...
inline void drawContours(cv::InputOutputArray image,
                         const std::vector<Contour_cptr>& contours, int contourIdx,
 ...
 */
static bool deleteContour(std::vector<Contour_cptr>& contours,
                          std::vector<cv::Vec4i>& hierarchy, int index)
{
  if (index < 0 || index >= contours.size()) return false;

  auto updateHierarchy = [&](int idx, int new_val, int pos)
  {
    if (idx != -1) hierarchy[idx][pos] = new_val;
  };
