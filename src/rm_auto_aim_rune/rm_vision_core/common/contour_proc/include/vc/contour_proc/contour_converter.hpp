 ...
 */
template <typename T>
concept ContourWrapperConvertable = requires(T t) { requires ContourWrapperPtrType<T>; };
/**
 * @brief 轮廓转化模块
 *
 * @note 该模块用于将传入的各种格式的轮廓数据转换为 std::shared_ptr<ContourWrapper<int>>
 * 对象
 */
class ContourConverter
{
 private:
  /**
   * @brief 将传入的 ContourWrapperPtrType 类型转化为 std::shared_ptr<ContourWrapper<int>>
   * 对象
   */
