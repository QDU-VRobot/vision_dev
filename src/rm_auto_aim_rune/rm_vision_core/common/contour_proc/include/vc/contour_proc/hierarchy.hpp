 ...
 */
template <typename T>
void getAllSubContoursIdxImpl(const std::vector<cv::Vec4i>& hierarchy, int idx,
                              T& sub_contours_idx)
{
  using namespace std;
  using namespace cv;

  if (hierarchy[idx][2] == -1) return;
  ...
  while (front_child_idx != -1)
  {
    ...
    getAllSubContoursIdxImpl(hierarchy, front_child_idx,
                             sub_contours_idx);       //!< 递归获取子轮廓
    front_child_idx = hierarchy[front_child_idx][1];  //!< 获取下一个兄弟轮廓下标
  }
  if (hierarchy[hierarchy[idx][2]][0] == -1) return;
  ...
  while (back_child_idx != -1)
  {
    ...
    getAllSubContoursIdxImpl(hierarchy, back_child_idx,
                             sub_contours_idx);     //!< 递归获取子轮廓
 ...
 */
template <typename T>
void getAllSubContoursIdx(const std::vector<cv::Vec4i>& hierarchy, int idx,
                          T& sub_contours_idx)
{
