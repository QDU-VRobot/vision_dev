...
{
  return std::sqrt((__pt2.x - __pt1.x) * (__pt2.x - __pt1.x) +
...
template <typename _Tp>
typename Segment2_<_Tp>::point_type Segment2_<_Tp>::closestPoint(
...
template <typename _Tp>
std::vector<typename Segment2_<_Tp>::point_type> Segment2_<_Tp>::intersect(
    const Segment2_& other) const
{
  point_type p1 = __pt1;
  point_type q1 = __pt2;
  point_type p2 = other.__pt1;
