...
Circle2_<_Tp>::Circle2_() : __center(0, 0), __radius(1)
{
}
...
{
  if (radius <= 0) throw std::invalid_argument("圆的半径必须大于0");
}
...
Circle2_<_Tp>::Circle2_(const point_type& p1, const point_type& p2, const point_type& p3)
{
  ...
  // 计算半径
  __radius = std::sqrt((p1.x - center_x) * (p1.x - center_x) +
...
{
  value_type d = std::sqrt((p.x - __center.x) * (p.x - __center.x) +
...
{
  value_type d_sq =
      (p.x - __center.x) * (p.x - __center.x) + (p.y - __center.y) * (p.y - __center.y);
...
template <typename _Tp>
std::vector<typename Circle2_<_Tp>::point_type> Circle2_<_Tp>::intersect(
    const Line2_<value_type>& line) const
{
  // 计算圆心到直线的垂直距离
    ...
    const Circle2_& other) const
{
  ...
  // 计算交点坐标
  point_type p0(__center.x + a * d_x / d, __center.y + a * d_y / d);
