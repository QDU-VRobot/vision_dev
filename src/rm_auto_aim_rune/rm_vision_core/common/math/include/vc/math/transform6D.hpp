 ...
 * @return 返回转换后的旋转矩阵
 */
Transform6D::RmatType convertRmat(
    const cv::Matx<Transform6D::RmatType::value_type, 3, 1>& rvec);
}  // namespace transform6D_utils

// 默认构造函数
inline Transform6D::Transform6D()
    : __tvec(TvecType(0, 0, 0)), __rmat(RmatType::eye()), __rvec(RvecType(0, 0, 0))
{
}
...
inline const Transform6D::TvecType& Transform6D::tvec() const noexcept { return __tvec; }
 ...
 * @return Transform6D::RmatType 转换后的旋转矩阵
 */
inline Transform6D::RmatType convertRmat(
    const cv::Matx<Transform6D::RvecType::value_type, 3, 1>& rvec)
{
