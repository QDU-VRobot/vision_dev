...
Line2_<_Tp>::Line2_() : __angle(0), __point(0, 0), __direction(1, 0)
{
}

template <typename _Tp>
Line2_<_Tp>::Line2_(value_type angle, const point_type& point)
    : __angle(angle), __point(point), __direction(std::cos(angle), std::sin(angle))
{
  normalizeAngle();
}

template <typename _Tp>
Line2_<_Tp>::Line2_(const point_type& p1, const point_type& p2) : __point(p1)
{
  if (p1 == p2) throw std::invalid_argument("直线的两个点不能相同");
...
template <typename _Tp>
std::vector<typename Line2_<_Tp>::point_type> Line2_<_Tp>::intersect(
    const Line2_& other) const
{
  const double tolerance = std::numeric_limits<value_type>::epsilon();
  const double det =
      __direction.x * other.__direction.y - __direction.y * other.__direction.x;

  if (std::abs(det) < tolerance) return {};
