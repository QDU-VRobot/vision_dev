 ...
 */
template <typename T>
concept vector_arithmetic =
    std::is_arithmetic_v<base_type<T>> &&
    (std::is_same_v<base_type<T>, int> || std::is_same_v<base_type<T>, float> ||
...
template <typename Target, typename Source>
  requires vector_3_type<Source> &&
           (std::same_as<std::remove_cvref_t<Target>,
                         cv::Vec<typename Target::value_type, 3>> ||
            std::same_as<std::remove_cvref_t<Target>,
                         cv::Matx<typename Target::value_type, 3, 1>>)
