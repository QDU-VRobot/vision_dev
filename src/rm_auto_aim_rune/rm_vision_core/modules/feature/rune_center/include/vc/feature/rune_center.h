...
class RuneCenter : public FeatureNode
{
 ...
 public:
   ...
   * @return 如果成功，返回 RuneCenter 的共享指针，否则返回 nullptr
   */
  static std::shared_ptr<RuneCenter> make_feature(
   ...
   */
  auto getPnpPoints() const -> std::tuple<std::vector<cv::Point2f>,
                                          std::vector<cv::Point3f>, std::vector<float>>;
  ...
  static inline const std::shared_ptr<const RuneCenter> cast(FeatureNode_cptr p_feature)
  {
    return std::dynamic_pointer_cast<const RuneCenter>(p_feature);
  }
