 ...
 * @return 等级结构是否满足要求
 */
inline bool isHierarchyCenter(const vector<Contour_cptr>& contours,
 ...
 */
inline void getSubContours(size_t outer_idx, const vector<Contour_cptr>& contours,
...
void RuneCenter::find(
    std::vector<FeatureNode_ptr>& centers, const std::vector<Contour_cptr>& contours,
...
shared_ptr<RuneCenter> RuneCenter::make_feature(
    const Contour_cptr& contour, const std::vector<Contour_cptr>& sub_contours)
{
  if (contour->points().size() < 6) return nullptr;
  // init
...
std::shared_ptr<RuneCenter> RuneCenter::make_feature(const PoseNode& center_to_cam)
{
  ...
  projectPoints(points_mat_3d, center_to_cam.rvec(), center_to_cam.tvec(),
                camera_param.cameraMatrix, camera_param.distCoeff, points_2d_reproject);
...
RuneCenter::getPnpPoints() const
{
  ...
  {
    VC_THROW_ERROR(" points_2d and points_3d are not equal. (size = %zu, %zu)",
                   points_2d.size(), points_3d.size());
  }
