...
RuneCombo_ptr RuneCombo::make_feature(const PoseNode& pnp_data, const RuneType& type,
                                      const GyroData& gyro_data, int64 tick)
{
  ...
  {
    target_pnp_data.tvec(pnp_data.tvec() +
                         pnp_data.rmat() * rune_target_param.TRANSLATION);
  ...
  if (type == RuneType::STRUCK)
  {
    fan_pnp_data.tvec(pnp_data.tvec() +
                      pnp_data.rmat() * rune_fan_param.ACTIVE_TRANSLATION);
  ...
  {
    fan_pnp_data.tvec(pnp_data.tvec() +
                      pnp_data.rmat() * rune_fan_param.INACTIVE_TRANSLATION);
 ...
 * @param p_fan 扇叶特征
 */
inline vector<Point2f> getRuneCorners(const FeatureNode_cptr& p_target,
                                      const FeatureNode_cptr& p_center,
                                      const FeatureNode_cptr& p_fan)
{
  // 获取击打区域（靶心）的半径
  ...
  projectPoints(corners_3d,
                p_target->getPoseCache().getPoseNodes().at(CoordFrame::CAMERA).rvec(),
                p_target->getPoseCache().getPoseNodes().at(CoordFrame::CAMERA).tvec(),
                camera_param.cameraMatrix, camera_param.distCoeff, corners_2d);
  return corners_2d;
}

RuneCombo::RuneCombo(const FeatureNode_ptr& p_target, const FeatureNode_ptr& p_center,
                     const FeatureNode_ptr& p_fan, const PoseNode& rune_to_cam,
                     const RuneType& type, const GyroData& gyro_data, int64 tick)
{
  ...
  Matx33f joint_to_gyro_R = euler2Mat(deg2rad(gyro_data.rotation.yaw), Y) *
                            euler2Mat(deg2rad(-1 * gyro_data.rotation.pitch), X);
