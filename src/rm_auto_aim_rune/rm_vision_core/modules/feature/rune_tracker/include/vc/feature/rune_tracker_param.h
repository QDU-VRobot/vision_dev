...
struct RuneTrackerParam
{
  ...
  YML_INIT(RuneTrackerParam, YML_ADD_PARAM(TRACK_FRAMES); YML_ADD_PARAM(SAMPLE_INTERVAL);
           YML_ADD_PARAM(ROTATE_Q); YML_ADD_PARAM(ROTATE_R); YML_ADD_PARAM(MOTION_Q);
           YML_ADD_PARAM(MOTION_R); YML_ADD_PARAM(MAX_DEQUE_SIZE););
};
