...
BottomCenterHump::BottomCenterHump(const Point2f& _center, const Point2f& direction,
                                   int _idx)
{
  center = _center;
...
vector<BottomCenterHump> BottomCenterHump::getBottomCenterHump(
    const vector<Point>& contour, const Point2f& contour_center,
    const vector<TopHump>& top_humps)
{
  if (contour.size() < 30) return {};
  if (top_humps.size() != 3)
    VC_THROW_ERROR("Size of the \" top_humps \" are not equal to 3. (size = %zu)",
                   top_humps.size());
...
bool BottomCenterHump::getAllHumps(const vector<Point>& contour,
                                   const vector<TopHump>& top_humps,
                                   vector<BottomCenterHump>& humps)
{
  ...
  if (found_humps.empty()) return false;
  humps = move(found_humps);
  return true;
}

bool BottomCenterHump::filter(const vector<Point>& contour,
                              const vector<TopHump>& top_humps,
                              vector<BottomCenterHump>& humps)
{
  ...
  for (auto it1 = humps.begin(); it1 != humps.end(); it1++)
  {
    ...
    {
      if (getDist(it1->getCenter(), it2->getCenter()) <
          rune_fan_hump_param.BOTTOM_CENTER_HUMP_MIN_INTERVAL)
        it2 = humps.erase(it2);
      else
        it2++;
    }
  ...
  humps = {*bottom_it};
  float delta_angle =
      getVectorMinAngle(line_direction, humps[0].getCenter() - line_center, DEG);
