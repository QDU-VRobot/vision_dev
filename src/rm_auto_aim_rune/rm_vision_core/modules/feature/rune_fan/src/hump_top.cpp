...
vector<TopHump> TopHump::getTopHumps(const vector<Point>& contour_plus,
                                     ...
                                     const vector<tuple<Line, Line>>& line_pairs)
{
  ...
  {
    VC_WARNING_INFO(
        "fan active get_top_humps : 轮廓长度小于20,无法进行查找,轮廓数量：%zu",
        contour_plus.size());
    return {};
  }
  ...
  {
    VC_WARNING_INFO(
        "fan active get_top_humps : 收集到的突起点数不足3个,无法进行查找,轮廓数量：%zu",
        contour_plus.size());
    return {};
  }
  ...
  {
    VC_WARNING_INFO(
        "fan active get_top_humps : 过滤后的突起点数不足3个,无法进行查找,轮廓数量：%zu",
        contour_plus.size());
    return {};
  }
...
inline bool TopHump::getAllHumps2(const vector<Point>& contours_plus,
                                  const vector<tuple<Line, Line>>& line_pairs,
                                  vector<TopHump>& humps)
{
  humps.clear();
...
bool TopHump::filter(const vector<Point>& contour_plus, vector<TopHump>& humps)
{
  ...
  for (auto it_1 = humps.begin(); it_1 != humps.end(); it_1++)
    ...
    {
      if (getVectorMinAngle(it_1->getDirection(), it_2->getDirection(), DEG) <
          rune_fan_hump_param.TOP_HUMP_MAX_ALIGNMENT_DELTA)
      {
        if (getDist(it_1->getVertex(), it_2->getVertex()) <
...
bool TopHump::make_TopHumps(TopHumpCombo& hump_combo, const Point2f& contour_center,
                            double& delta)
{
  ...
  auto center_hump_it = min_element(humps_to_classify.begin(), humps_to_classify.end(),
                                    ...
                                    {
                                      return getDist(h1.getVertex(), aveVertex) <
                                             getDist(h2.getVertex(), aveVertex);
                                    });
  center_h = *center_hump_it;
  humps_to_classify.erase(center_hump_it);
  if (humps_to_classify.size() != 2) return false;
  Point2f c0 = humps_to_classify[0].getVertex() - contour_center,
  ...
  if (getVectorMinAngle(center_h.getDirection(), center_h.getVertex() - contour_center,
  ...
  double distance_ratio = getDist(left_h.getVertex(), center_h.getVertex()) /
                          getDist(center_h.getVertex(), right_h.getVertex());
  ...
  Vec4f line;
  fitLine(vector<Point2f>{left_h.getVertex(), center_h.getVertex(), right_h.getVertex()},
          line, DIST_L2, 0, 0.01, 0.01);
  Point2f ave_direction =
      (left_h.getDirection() + center_h.getDirection() + right_h.getDirection()) / 3.0;
  double line_direction_delta =
      getVectorMinAngle(Point2f(line[0], line[1]), ave_direction, DEG);
...
bool TopHump::setVertex(TopHump& hump, const vector<Point>& contour_plus)
{
  ...
  auto farthest_point = max_element(
      ...
      {
        return getProjection(Point2f(p1) - hump.getCenter(), hump.getDirection()) <
               getProjection(Point2f(p2) - hump.getCenter(), hump.getDirection());
      });
  hump.vertex =
      hump.getCenter() + getProjectionVector(Point2f(*farthest_point) - hump.getCenter(),
                                             hump.getDirection());
  return true;
}
