...
inline bool getHumps(const vector<Point>& contour, vector<TopHump>& top_humps)
{
  ...
  for (auto& h : humps) TopHump::setVertex(h, contour_plus);
    ...
    unordered_map<FeatureNode_cptr, unordered_set<size_t>>& used_contour_idxs)
{
  ...
  if (pending_idxs.empty()) return false;
  ...
  static auto getContourIdx = [](const Contour_cptr& c,
                                 const vector<Contour_cptr>& contours) -> int
  {
    auto it = std::find(contours.begin(), contours.end(), c);
    if (it != contours.end()) return std::distance(contours.begin(), it);
  ...
  for (auto idx : pending_idxs)
  {
    ...
    for (auto& h : temp_humps) all_humps.push_back({h, contours[idx]});
  }
  if (all_humps.size() < 3) return false;
  ...
  for (size_t i = 0; i < all_humps.size(); i++)
  {
    ...
    for (size_t n = 0; n < found; ++n)
    {
      ...
      if (j <= i || sqrt(dists.at<float>(0, n)) > 300) continue;
      auto &h1 = get<0>(all_humps[i]), &h2 = get<0>(all_humps[j]);
      if (getVectorMinAngle(h1.getDirection(), h2.getDirection(), DEG) > 30) continue;

      for (size_t n2 = 0; n2 < found; ++n2)
      {
        size_t k = indices.at<int>(0, n2);
        if (k <= i || k == j || sqrt(dists.at<float>(0, n2)) > 300) continue;
        auto& h3 = get<0>(all_humps[k]);
        if (getVectorMinAngle(h1.getDirection(), h3.getDirection(), DEG) > 30) continue;
        if (getVectorMinAngle(h2.getDirection(), h3.getDirection(), DEG) > 30) continue;
        ...
        if (p_fan)
        {
          ...
          used_contour_idxs_temp[p_fan].insert(
              getContourIdx(get<1>(all_humps[i]), contours));
          used_contour_idxs_temp[p_fan].insert(
              getContourIdx(get<1>(all_humps[j]), contours));
          used_contour_idxs_temp[p_fan].insert(
              getContourIdx(get<1>(all_humps[k]), contours));
        }
      }
    }
  }
...
RuneFanActive::RuneFanActive(const vector<Contour_cptr>& contours,
                             const vector<Point2f>& top_hump_corners,
                             const Point2f& direction)
{
  vector<Point> contour_temp;
  for (auto& c : contours) contour_temp.insert(contour_temp.end(), c->begin(), c->end());
...
inline Point2f getHumpCenter(const array<tuple<TopHump, Contour_cptr>, 3>& humps)
{
  ...
  for (auto& [h, c] : humps) contours.insert(c);
  if (contours.size() == 3)
  {
    Point2f ave_point = (get<0>(humps[0]).getVertex() + get<0>(humps[1]).getVertex() +
                         get<0>(humps[2]).getVertex()) /
                        3.0;
    array<float, 3> distances;
    for (int i = 0; i < 3; i++)
      distances[i] = getDist(ave_point, get<0>(humps[i]).getVertex());
    return get<1>(humps[distance(distances.begin(),
...
shared_ptr<RuneFanActive> RuneFanActive::make_feature(
    const tuple<TopHump, Contour_cptr>& h1, const tuple<TopHump, Contour_cptr>& h2,
    const tuple<TopHump, Contour_cptr>& h3)
{
  ...
  if (max_distance == 0) return nullptr;

  double max_side_length = max(
      {max(contour_1->fittedEllipse().size.width, contour_1->fittedEllipse().size.height),
       max(contour_2->fittedEllipse().size.width, contour_2->fittedEllipse().size.height),
       max(contour_3->fittedEllipse().size.width,
           contour_3->fittedEllipse().size.height)});
  if (max_side_length == 0 || max_distance > 3.0 * max_side_length) return nullptr;

  double angle_delta = 1e5;
  if (!TopHump::make_TopHumps(hump_combo, contours_center, angle_delta)) return nullptr;
  ...
  for (auto& h : top_humps) top_humps_corners.push_back(h.getVertex());

  auto fan = make_shared<RuneFanActive>(fan_contours, top_humps_corners, direction);
  if (fan && !fan->isSetError()) fan->setError(angle_delta);
  return fan;
}
