      ...
      end_idx(_end_idx)
{
  ...
  avePoint = accumulate(points.begin(), points.end(), Point2f(0, 0),
                        [](Point2f sum, const Point& p)
...
double HumpDetector::CheckCollinearity(const Point2f& p1, const Point2f& p2,
                                       const Point2f& p3)
{
...
double HumpDetector::CheckAlignment(const Point2f& dir1, const Point2f& dir2,
                                    const Point2f& dir3)
{
  ...
  for (int i = 0; i < 3; i++)
    while (aligned[i] < -M_PI) aligned[i] += 2 * M_PI;
  for (int i = 0; i < 3; i++)
    while (aligned[i] > M_PI) aligned[i] -= 2 * M_PI;

  double mean = (aligned[0] + aligned[1] + aligned[2]) / 3.0;
  double var = 0.0;
  for (int i = 0; i < 3; i++) var += (aligned[i] - mean) * (aligned[i] - mean);
