...
vector<SideHump> SideHump::getSideHumps(
    const vector<Point>& contour_plus, const Point2f& contour_center,
    const vector<TopHump>& top_humps, const vector<BottomCenterHump>& bottom_center_humps,
                           ...
                           vector<SideHump>& humps)
{
  ...
  auto isSideHump = [&](const Point2f& top_center, const Point2f& top_dir,
                        const Point2f& center, const Point2f& dir)
  {
    if (getVectorMinAngle(top_dir, dir, DEG) >
    ...
    if (getVectorMinAngle(v1, v2, DEG) >
            rune_fan_hump_param.SIDE_HUMP_MAX_ANGLE_DELTA / 2.0 ||
        getVectorMinAngle(v2, v3, DEG) >
            rune_fan_hump_param.SIDE_HUMP_MAX_ANGLE_DELTA / 2.0)
      return false;
    return true;
  };
  ...
  for (auto& idx : line_pair_idx_set)
  {
    ...
    {
      humps.emplace_back(
          dir, getLineIntersection(l_center, l_center + l_dir, center, center + dir));
    }
    else if (to_r <= to_l && isSideHump(r_center, r_dir, center, dir))
    {
      humps.emplace_back(
          dir, getLineIntersection(r_center, r_center + r_dir, center, center + dir));
    }
  }
...
// 确定左右侧突起点
bool SideHump::make_SideHumps(vector<SideHump>& humps, const vector<TopHump>& top_humps,
                              const Point2f& contour_center, double& delta)
{
  if (humps.size() != 2)
  {
  ...
  if (cross0 < 0) swap(humps[0], humps[1]);
  delta = 0;
  return true;
}
