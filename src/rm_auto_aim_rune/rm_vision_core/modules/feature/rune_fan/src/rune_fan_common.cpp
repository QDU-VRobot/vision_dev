...
void RuneFan::find_inactive_fans(
    std::vector<FeatureNode_ptr>& fans, const std::vector<Contour_cptr>& contours,
    const std::vector<cv::Vec4i>& hierarchy, const std::unordered_set<size_t>& mask,
    std::unordered_map<FeatureNode_cptr, std::unordered_set<size_t>>& used_contour_idxs,
...
void RuneFan::find_incomplete_active_fans(
    std::vector<FeatureNode_ptr>& fans, const std::vector<Contour_cptr>& contours,
    const std::vector<cv::Vec4i>& hierarchy, const std::unordered_set<size_t>& mask,
    const cv::Point2f& rotate_center,
    std::unordered_map<FeatureNode_cptr, std::unordered_set<size_t>>& used_contour_idxs)
{
  RuneFanActive::find_incomplete(fans, contours, hierarchy, mask, rotate_center,
                                 used_contour_idxs);
}
    ...
    -> std::tuple<std::vector<cv::Point2f>, std::vector<cv::Point3f>, std::vector<float>>
{
  ...
  for (int i = 0; i < points_3d.size(); i++)
  {
    ...
    relative_points_3d[i] = Point3f(relative_points_3d_mat(0), relative_points_3d_mat(1),
                                    relative_points_3d_mat(2));
  }
...
std::shared_ptr<RuneFan> RuneFan::make_feature(const PoseNode& fan_to_cam, bool is_active)
{
  ...
  if (is_active)
  {
    ...
    projectPoints(top_hump_corners, fan_to_cam.rvec(), fan_to_cam.tvec(),
                  camera_param.cameraMatrix, camera_param.distCoeff, top_hump_corners_2d);
    projectPoints(bottom_center_hump_corners, fan_to_cam.rvec(), fan_to_cam.tvec(),
                  camera_param.cameraMatrix, camera_param.distCoeff,
                  bottom_center_hump_corners_2d);
    projectPoints(side_hump_corners, fan_to_cam.rvec(), fan_to_cam.tvec(),
                  camera_param.cameraMatrix, camera_param.distCoeff,
                  side_hump_corners_2d);
    projectPoints(bottom_side_hump_corners, fan_to_cam.rvec(), fan_to_cam.tvec(),
                  camera_param.cameraMatrix, camera_param.distCoeff,
                  bottom_side_hump_corners_2d);

    if (checkPoints(top_hump_corners_2d) == false ||
        checkPoints(bottom_center_hump_corners_2d) == false ||
        checkPoints(side_hump_corners_2d) == false ||
        checkPoints(bottom_side_hump_corners_2d) == false)
    {
      return nullptr;
    }

    result_ptr =
        RuneFanActive::make_feature(top_hump_corners_2d, bottom_center_hump_corners_2d,
  ...
  else
  {
    ...
    projectPoints(corners, fan_to_cam.rvec(), fan_to_cam.tvec(),
    ...
    result_ptr = RuneFanInactive::make_feature(corners_2d[0], corners_2d[1],
                                               corners_2d[2], corners_2d[3]);
  }
  if (result_ptr)
  {
