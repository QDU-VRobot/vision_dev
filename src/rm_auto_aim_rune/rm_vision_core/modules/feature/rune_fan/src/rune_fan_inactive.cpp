...
inline bool isHierarchyInactiveFan(const vector<Contour_cptr>& contours,
...
inline void filterFanContours(const std::vector<Contour_cptr>& in_contours,
                              std::vector<Contour_cptr>& out_contours,
                              vector<FeatureNode_ptr>& inactive_targets)
{
  if (in_contours.size() < 2 || inactive_targets.empty()) return;
  ...
  if (inactive_targets.size() > 1)  // 多个靶心，选择最近的那个
  {
    ...
    double area_sum = 0;
    for (auto& contour : in_contours) area_sum += contour->area();
    ...
    auto near_target =
        *min_element(inactive_targets.begin(), inactive_targets.end(),
                     [&](const FeatureNode_ptr& a, const FeatureNode_ptr& b)
                     {
  ...
  auto far_contour = *max_element(in_contours.begin(), in_contours.end(),
                                  [&](const Contour_cptr& a, const Contour_cptr& b)
                                  {
  ...
  out_contours.erase(
      remove_if(out_contours.begin(), out_contours.end(),
                [&](const Contour_cptr& contour) { return contour == far_contour; }),
      out_contours.end());
}

void RuneFanInactive::find(
    std::vector<FeatureNode_ptr>& fans, const std::vector<Contour_cptr>& contours,
    ...
    const std::vector<FeatureNode_cptr>& inactive_targets)
{
  ...
  for (auto i = 0; i < contours.size(); i++)
  {
    ...
    if (isHierarchyInactiveFan(contours, hierarchy, i) == false) continue;
  ...
  for (auto& group : contours_group)
  {
    ...
    for (auto& idx : group) temp_contours.push_back(contours[idx]);
...
RuneFanInactive::RuneFanInactive(const Contour_cptr hull_contour,
                                 const vector<Contour_cptr>& arrow_contours,
                                 ...
                                 const Point2f& bottom_right, const Point2f& bottom_left)
{
  ...
  auto contour = ContourWrapper<int>::make_contour(
  ...
  image_info.setDirection(
      getUnitVector(bottom_center - top_center));  // 扇叶的方向指向神符中心
}

bool RuneFanInactive::correctDirection(FeatureNode_ptr& fan,
                                       const cv::Point2f& correct_center)
{
  auto rune_fan = RuneFanInactive::cast(fan);
  if (rune_fan == nullptr) return false;
  if (rune_fan->getActiveFlag()) return false;
  if (rune_fan->getImageCache().getDirection() == cv::Point2f(0, 0)) return false;
  if (correct_center == cv::Point2f(0, 0)) return false;
  ...
  Point2f reference_direction =
      getUnitVector(correct_center - rune_fan->getImageCache().getCenter());
  if (direction.dot(reference_direction) < 0) direction = -direction;
...
bool RuneFanInactive::correctCorners(FeatureNode_ptr& fan)
{
  ...
  if (v0.cross(v1) < 0) swap(temp_corners[0], temp_corners[1]);
  if (v2.cross(v3) < 0) swap(temp_corners[2], temp_corners[3]);

  rune_fan->getImageCache().setCorners(temp_corners);
  return true;
}

Contour_cptr RuneFanInactive::getEndArrowContour(
    FeatureNode_ptr& inactive_fan, const vector<FeatureNode_cptr>& inactive_targets)
{
  // 0. 判空
  if (inactive_targets.empty()) return nullptr;
  if (inactive_fan == nullptr) return nullptr;
  ...
  else
  {
    unordered_map<Contour_cptr, double> contour_weights{};
    double area_sum = 0;
    for (auto& contour : arrow_contours) area_sum += contour->area();

    if (area_sum == 0) return nullptr;
    ...
    auto near_target =
        *min_element(inactive_targets.begin(), inactive_targets.end(),
                     [&](const FeatureNode_cptr& a, const FeatureNode_cptr& b)
                     {
                       return norm(a->getImageCache().getCenter() - all_contours_center) <
                              norm(b->getImageCache().getCenter() - all_contours_center);
                     });
                                  ...
                                  const FeatureNode::DrawConfig_cptr& config) const
{
  ...
  do
  {
    ...
    if (!image_info.isSetCorners()) break;
  ...
  do
  {
    if (!getImageCache().isSetDirection()) break;
