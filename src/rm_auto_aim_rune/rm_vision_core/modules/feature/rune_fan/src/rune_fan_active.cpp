...
inline bool isHierarchyActiveFan(const vector<Contour_cptr>& contours,
...
void RuneFanActive::find(
    vector<FeatureNode_ptr>& fans, const vector<Contour_cptr>& contours,
    ...
    unordered_map<FeatureNode_cptr, unordered_set<size_t>>& used_contour_idxs)
{
  ...
  {
    if (mask.count(i) || contours[i]->points().size() < 6) continue;
...
shared_ptr<RuneFanActive> RuneFanActive::make_feature(const Contour_cptr& contour)
{
  ...
  if (side_ratio > rune_fan_param.ACTIVE_MAX_SIDE_RATIO) return nullptr;
  ...
  vector<Point2f> top_hump_corners, bottom_center_hump_corners, side_hump_corners,
...
RuneFanActive_ptr RuneFanActive::make_feature(
    const vector<Point2d>& top_corners, const vector<Point2d>& bottom_center_corners,
    const vector<Point2d>& side_corners, const vector<Point2d>& bottom_side_corners)
{
  ...
  vector<Point2f> top_corners_f, bottom_center_corners_f, side_corners_f,
  ...
  auto contour =
      ContourWrapper<int>::make_contour(vector<Point>(hull.begin(), hull.end()));
...
Mat RuneFanActive::getAngles(const vector<Point>& contour_plus)
{
  ...
  Mat directions_mat(contours_mat.size(), CV_32F),
      direction_kernel = (Mat_<float>(1, 2) << -1, 1);
  filter2D(contours_mat, directions_mat, -1, direction_kernel, Point(0, 0), 0,
           BORDER_DEFAULT);
  int filter_len =
      max(17, min(static_cast<int>(rune_fan_hump_param.TOP_HUMP_FILTER_LEN_RATIO *
                                   contour_plus.size()) |
                      1,
                  101));
  Mat kernel =
      getGaussianKernel(filter_len, rune_fan_hump_param.TOP_HUMP_FILTER_SIGMA, CV_32F)
          .t();
...
// 获取所有的线段
inline bool getAllLine(const vector<Point>& contour_plus, const Mat& angles_mat,
                       const Mat& gradient, std::vector<Line>& lines)
{
  ...
  {
    float angle = accumulate(angles_mat.ptr<float>(0) + s_idx,
                             angles_mat.ptr<float>(0) + e_idx + 1, 0.0f) /
                  (e_idx - s_idx + 1);
    Point2f center =
        static_cast<Point2f>(accumulate(contour_plus.begin() + s_idx,
                                        contour_plus.begin() + e_idx + 1, Point(0, 0))) /
        float(e_idx - s_idx + 1);
...
inline bool matchLine(const vector<Point> contour_plus, const Mat& angles_mat,
                      std::vector<Line>& lines,
                      std::vector<tuple<Line, Line>>& matched_lines)
{
  ...
  for (size_t i = 0; i < lines.size(); i++)
  {
    ...
    {
      if (abs(lines[i].angle - lines[j].angle) > 360) break;
      if (abs(lines[i].end_idx - lines[j].start_idx) > contour_plus.size() / 3.0) break;
      float delta_angle = abs(lines[j].angle - lines[i].angle);
      if (abs(delta_angle - 180) > max_angle_delta) continue;
      float v_dist = getLineVerticalDistance(lines[i], lines[j]);
      if (v_dist > max_vertical_distance || v_dist > last_vertical_distance) continue;
      last_vertical_distance = v_dist;
      matched_j = j;
    }
    if (matched_j != size_t(-1)) matched_idxs.emplace_back(i, matched_j);
  }
  ...
  for (size_t i = 0; i < matched_idxs.size(); i++)
  {
    ...
    for (size_t j = i + 1; j < matched_idxs.size(); j++)
    {
      ...
      if (down1 == down2)
      {
        ...
        {
          l1.angle = (l1.angle * (l1.end_idx - l1.start_idx + 1) +
                      l2.angle * (l2.end_idx - l2.start_idx + 1)) /
  ...
  matched_idxs.erase(
      remove_if(matched_idxs.begin(), matched_idxs.end(),
                [&](const tuple<size_t, size_t>& idx)
                {
...
// 重合线段对合并
inline bool mergeLinePairs(const vector<Point>& contour_plus,
                           std::vector<std::tuple<Line, Line>>& line_pairs)
{
  ...
  for (int i = 0; i < static_cast<int>(line_pairs.size()); i++)
  {
    ...
    for (int j = i + 1; j < static_cast<int>(line_pairs.size()); j++)
    {
      ...
      if (getDist(center1, center2) > max_distance) continue;

      float up_delta = getDeltaAngle(up1, up2);
      float down_delta = getDeltaAngle(down1, down2);
      if (up_delta > max_angle_delta || down_delta > max_angle_delta) continue;
      ...
      {
        Point2f dir =
            (Point2f(cos(deg2rad(up1.angle)), sin(deg2rad(up1.angle))) * up_len1 +
      ...
      {
        Point2f dir =
            (Point2f(cos(deg2rad(down1.angle)), sin(deg2rad(down1.angle))) * down_len1 +
...
// 占位线段延长
inline bool extendLines(const vector<Point>& contour_plus, const std::vector<Line>& lines,
                        std::vector<tuple<Line, Line>>& matched_idxs)
{
  return true;
}

// 获取线段对主流程
bool RuneFanActive::getLinePairs(const vector<Point>& contour_plus, const Mat& angles_mat,
                                 const Mat& gradient_mat,
                                 std::vector<std::tuple<Line, Line>>& line_pairs)
{
  vector<Line> lines;
...
bool RuneFanActive::getActiveFunCorners(
    const Contour_cptr& contour, std::vector<cv::Point2f>& top_hump_corners,
    ...
    std::vector<cv::Point2f>& bottom_side_hump_corners)
{
  ...
  if (line_pairs.size() < 4) return false;

  auto top_humps = TopHump::getTopHumps(contour_plus, center, line_pairs);
  if (top_humps.empty()) return false;
  auto bottom_center_humps =
      BottomCenterHump::getBottomCenterHump(contour_plus, center, top_humps);
  if (bottom_center_humps.empty()) return false;
  auto side_humps = SideHump::getSideHumps(contour_plus, center, top_humps,
                                           bottom_center_humps, line_pairs);

  top_hump_corners = top_humps.empty() ? vector<Point2f>{}
                                       : vector<Point2f>{top_humps[0].getVertex(),
                                                         top_humps[1].getVertex(),
                                                         top_humps[2].getVertex()};
  bottom_center_hump_corners = bottom_center_humps.empty()
                                   ? vector<Point2f>{}
                                   : vector<Point2f>{bottom_center_humps[0].getVertex()};
  side_hump_corners = side_humps.empty() ? vector<Point2f>{}
                                         : vector<Point2f>{side_humps[0].getVertex(),
...
{
  if (!top_hump_corners.empty()) setTopHumpCorners(top_hump_corners);
  if (!bottom_center_hump_corners.empty())
    setBottomCenterHumpCorners(bottom_center_hump_corners);
  if (!side_hump_corners.empty()) setSideHumpCorners(side_hump_corners);
...
void RuneFanActive::drawFeature(cv::Mat& image,
                                const FeatureNode::DrawConfig_cptr& config) const
{
  ...
  if (img_info.isSetCenter() && img_info.isSetDirection())
  {
    ...
    if (dir != Point2f(0, 0))
      cv::arrowedLine(image, center,
                      ...
                      rune_fan_draw_param.active.arrow_thickness, LINE_AA, 0, 0.1);
  }
}
