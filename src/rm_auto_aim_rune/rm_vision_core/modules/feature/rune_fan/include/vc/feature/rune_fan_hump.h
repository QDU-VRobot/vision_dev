...
class TopHump : public RuneFanHump
{
 ...
 public:
  ...
  static bool make_TopHumps(TopHumpCombo& hump_combo, const cv::Point2f& contour_center,
                            double& delta);
...
struct TopHumpCombo
{
  ...
  Contour_cptr getContour() const
  {
    if (__contour_ptr == nullptr)
    {
      __contour_ptr = ContourWrapper<int>::getConvexHull(
          {__humps[0].getContour(), __humps[1].getContour(), __humps[2].getContour()});
    }
    return __contour_ptr;
  }
...
class SideHump : public RuneFanHump
{
 ...
 private:
  ...
  static bool make_SideHumps(std::vector<SideHump>& humps,
                             const std::vector<TopHump>& top_humps,
