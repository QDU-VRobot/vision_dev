...
class RuneFanInactive : public RuneFan
{
  ...
  DEFINE_PROPERTY(ArrowContours, public, protected,
                  (std::vector<Contour_cptr>));  //!< 箭头轮廓组
 ...
 public:
   ...
   */
  RuneFanInactive(const Contour_cptr hull_contour,
                  const std::vector<Contour_cptr>& arrow_contours,
   ...
   */
  RuneFanInactive(const cv::Point2f& top_left, const cv::Point2f& top_right,
                  const cv::Point2f& bottom_right, const cv::Point2f& bottom_left);
   ...
   */
  static Contour_cptr getEndArrowContour(
      FeatureNode_ptr& fan, const std::vector<FeatureNode_cptr>& target_inactive);
