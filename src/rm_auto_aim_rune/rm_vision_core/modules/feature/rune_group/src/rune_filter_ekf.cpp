...
RuneFilterEKF_CV::RuneFilterEKF_CV(RuneFilterDataType type) : _data_type(type)
{
  ...
  for (int i = 6; i < 12; i++) P_.at<double>(i, i) = rune_ekf_param.INIT_VEL_VAR;
  ...
  for (int i = 0; i < 6; i++) H_.at<double>(i, i) = 1.0;
}
...
RuneFilterStrategy::FilterOutput RuneFilterEKF_CV::filter(FilterInput input)
{
  if (!initialized_)
  {
    for (int i = 0; i < 6; i++) x_.at<double>(i, 0) = input.raw_pos(i);
  ...
  dt_ = max((input.tick - last_tick_) * 1e-9, rune_ekf_param.MIN_DT);
  last_tick_ = input.tick;
  ...
  {
    Mat z(6, 1, CV_64F);
    for (int i = 0; i < 6; i++) z.at<double>(i, 0) = input.raw_pos(i);
    update(z);
  }

  __filter_count++;
  for (int i = 0; i < 6; i++) __latest_value(i) = x_.at<double>(i, 0);
...
{
  Matx61d pred;
  for (int i = 0; i < 6; i++) pred(i) = x_.at<double>(i, 0);
  return pred;
}

Matx61d RuneFilterEKF_CV::predictAhead(double dt_future) const
{
  // Constant velocity model: x_future = F(dt_future) * x_
  cv::Mat F = cv::Mat::eye(12, 12, CV_64F);
  for (int i = 0; i < 3; i++)
  {
    F.at<double>(i, 6 + i) = dt_future;
    F.at<double>(3 + i, 9 + i) = dt_future;
  }

  cv::Mat x_future = F * x_;
  cv::Matx61d pred;
  for (int i = 0; i < 6; i++) pred(i) = x_future.at<double>(i, 0);
  return pred;
}

bool RuneFilterEKF_CV::isValid() { return initialized_; }
