...
auto RuneFilterFusion::filter(FilterInput input) -> FilterOutput
{
  ...
  else
  {
    ...
    for (auto& filt : _filters) type_map[filt->getDataType()].push_back(filt);
...
Matx61d RuneFilterFusion::getPredict()
{
  ...
  if (!x || !y || !r) VC_THROW_ERROR("滤波器未初始化");
  if (!x->isValid() || !y->isValid() || !r->isValid()) VC_THROW_ERROR("滤波器无效");
...
{
  if (_filters.size() == 1) return _filters.front()->isValid();
