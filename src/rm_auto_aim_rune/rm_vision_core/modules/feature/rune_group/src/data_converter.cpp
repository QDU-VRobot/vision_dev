...
{
  auto rvec =
      euler2Rvec(deg2rad(filter_pos(3)), deg2rad(filter_pos(4)), deg2rad(filter_pos(5)));
  return {Matx31f{filter_pos(0), filter_pos(1), filter_pos(2)}, rvec};
}

Matx61f DataConverter::toFilterForm(const Vec3f& tvec, const Vec3f& rvec,
                                    bool& is_gimbal_lock, bool is_reset)
{
  ...
  {
    while (yaw - _last_yaw > CV_PI) yaw -= 2 * CV_PI;
    while (yaw - _last_yaw < -CV_PI) yaw += 2 * CV_PI;
    _last_yaw = yaw;

    while (roll - _last_roll > CV_PI) roll -= 2 * CV_PI;
    while (roll - _last_roll < -CV_PI) roll += 2 * CV_PI;
    _last_roll = roll;
  }
...
{
  Matx33f rmat = euler2Mat(yaw, EulerAxis::Y) * euler2Mat(pitch, EulerAxis::X) *
                 euler2Mat(roll, EulerAxis::Z);
  Matx31f rvec;
  Rodrigues(rmat, rvec);
  return rvec;
}
