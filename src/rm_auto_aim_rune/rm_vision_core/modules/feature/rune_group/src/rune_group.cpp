...
{
  for (int i = 0; i < 6; i++)
    if (isnan(pos(i)) || isinf(pos(i)) || abs(pos(i)) > 1e5) return false;
  return true;
}
...
{
  float diff[6];
  for (int i = 0; i < 6; i++) diff[i] = p1(i) - p2(i);
...
inline bool checkPoseValid(const PoseNode& pose)
{
  ...
  for (int i = 0; i < 3; i++)
    if (isnan(t(i)) || isnan(r(i)) || isinf(t(i)) || isinf(r(i))) return false;
  return true;
}
...
bool RuneGroup::checkExtremePose(const PoseNode& r_cam, const PoseNode& r_gyro,
                                 const GyroData& gyro)
{
  if (!rune_group_param.ENABLE_EXTREME_VALUE_FILTER) return true;
  if (!checkPoseValid(r_cam) || !checkPoseValid(r_gyro)) return false;
...
{
  clearPreviousData();
  if (!_filter) _filter = RuneFilterFusion::make_filter();
  ...
  if (!vanish) setLastValidPoseGyro(r_gyro_raw);
  ...
  Matx61f raw_pos =
      converters["4"]->toFilterForm(r_gyro_raw.tvec(), r_gyro_raw.rvec(), gimbal);

  if (_filter->isValid() && !checkPoseDiff(_filter->getPredict(), raw_pos)) return false;
  if (!checkExtremePose(r_cam_raw, r_gyro_raw, gyro)) return false;
  if (!errorValueProcess(raw_pos)) return false;
  ...
  if (!vanish) updateCenterEstimation();
  return true;
}

bool RuneGroup::getCamPnpDataFromFilter(PoseNode& rune_to_cam) const
{
  if (!_filter || !_filter->isValid()) return false;
...
{
  PoseNode pos_to_cam;
  if (!getCamPnpDataFromFilter(pos_to_cam)) return false;
  ...
  if (isSetPredictFunc() && getPredictFunc())
  {
    ...
    if (ticks.size() >= 2)
    {
      ...
      if (!std::isnan(angle) && !std::isinf(angle)) roll = angle;
    }
  }
...
std::vector<RuneFeatureComboConst> RuneGroup::getLastFrameFeatures() const
{
  ...
  for (auto& tracker : getTrackers())
  {
    ...
    if (rune->getRuneType() == RuneType::UNKNOWN ||
        rune->getRuneType() == RuneType::UNSTRUCK)
      continue;
...
bool RuneGroup::visibilityProcess(bool obs)
{
  ...
  if (!obs)
  {
    if (++vn > rune_group_param.MAX_VANISH_NUMBER) return false;
  }
  else
    vn = 0;
  return true;
...
void RuneGroup::sync(const GyroData&, int64_t)
{
  ...
  auto& raw = getRawDatas();
  raw.push_front(roll);
  if (raw.size() > rune_group_param.RAW_DATAS_SIZE) raw.pop_back();
  auto& ticks = getHistoryTicks();
  ticks.push_front(getTick());
  if (ticks.size() > rune_group_param.RAW_DATAS_SIZE) ticks.pop_back();
}

inline unordered_set<size_t> getAbnormalFrames(
    const vector<TrackingFeatureNode_cptr>& trackers, size_t range)
{
  ...
  deque<vector<RuneType>> types(range,
                                vector<RuneType>(trackers.size(), RuneType::UNKNOWN));
  ...
  for (size_t f = 0; f < range; f++)
  {
    int cnt = 0;
    for (auto& r : types[f])
      if (r == RuneType::PENDING_STRUCK) cnt++;
    if (cnt == 0 || cnt > 1) abn.insert(f);
  }

  for (size_t f = 0; f < range - 1; f++)
  {
    if (abn.count(f) || abn.count(f + 1)) continue;
    int old_cnt = 0, new_cnt = 0;
    for (auto& r : types[f + 1])
      if (r == RuneType::STRUCK) old_cnt++;
    for (auto& r : types[f])
      if (r == RuneType::STRUCK) new_cnt++;
...
bool RuneGroup::isTargetCenterChanged() const
{
  ...
  for (auto& t : getTrackers()) trackers.push_back(TrackingFeatureNode::cast(t));
  if (trackers.size() != 5) return false;
  for (auto& t : trackers)
    if (t->getHistoryNodes().size() < 3) return false;

  int history_size = 10;
  vector<int> lens;
  for (auto& t : trackers) lens.push_back((int)t->getHistoryNodes().size());
  ...
  {
    if (abn.count(f)) continue;
    for (size_t t = 0; t < trackers.size(); t++)
    {
      auto combo = RuneCombo::cast(trackers[t]->getHistoryNodes()[f]);
      if (combo && combo->getRuneType() == RuneType::PENDING_STRUCK) pending_count[t]++;
    }
  }
...
{
  if (getTrackers().empty()) return false;
  const auto& tracker = TrackingFeatureNode::cast(getTrackers().front());
  if (tracker->getHistoryNodes().empty()) return false;
...
std::vector<FeatureNode_ptr> RuneGroup::getTrackers()
{
  ...
  for (auto& [id, tr] : getChildFeatures()) t.emplace_back(tr);
  return t;
}
...
std::string get_shared_id(const std::shared_ptr<T>& sp)
{
  ...
  oss << "obj-" << std::hex << std::setw(sizeof(uintptr_t) * 2) << std::setfill('0')
      << addr;
  return oss.str();
}
...
inline void drawCube(cv::Mat& img, const PoseNode& p, float x_len, float y_len,
                     float z_len, cv::Scalar c)
{
  float hx = x_len / 2, hy = y_len / 2, hz = z_len / 2;
  vector<Point3f> pts3d = {{-hx, -hy, -hz}, {hx, -hy, -hz}, {hx, hy, -hz}, {-hx, hy, -hz},
                           {-hx, -hy, hz},  {hx, -hy, hz},  {hx, hy, hz},  {-hx, hy, hz}};
  vector<Point2f> pts2d;
  projectPoints(pts3d, p.rvec(), p.tvec(), camera_param.cameraMatrix,
...
inline void drawPentagonalPrism(cv::Mat& img, const PoseNode& p, float radius,
                                float height, cv::Scalar c)
{
...
void RuneGroup::drawFeature(cv::Mat& img, const DrawConfig_cptr& config) const
{
  ...
  for (const auto& tracker : getTrackers()) tracker->drawFeature(img, config);
}
