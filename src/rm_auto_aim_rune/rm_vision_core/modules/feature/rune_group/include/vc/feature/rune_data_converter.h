...
class DataConverter
{
 public:
   ...
   */
  static std::tuple<cv::Matx31f, cv::Matx31f> toTvecAndRvec(
      const cv::Matx61f& filter_pos);
   ...
   */
  cv::Matx61f toFilterForm(const cv::Vec3f& tvec, const cv::Vec3f& rvec,
                           bool& is_gimbal_lock, bool is_reset = false);
   ...
   * @return cv::Matx61f 可滤波形式位姿 [x y z | yaw pitch roll]
   */
  static cv::Matx61f cvtPos(const cv::Vec3f& tvec, const cv::Vec3f& rvec,
                            bool& is_gimbal_lock);
