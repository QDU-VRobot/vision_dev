...
class RuneGroup : public TrackingFeatureNode
{
  ...
  DEFINE_PROPERTY_WITH_INIT(RawDatas, public, protected, (std::deque<double>),
                            {});  //!< 原始角度数据
  DEFINE_PROPERTY_WITH_INIT(AngleSpeeds, public, protected, (std::deque<double>),
                            {});  //!< 角速度数据（顺负逆正）
  DEFINE_PROPERTY(PredictFunc, public, public,
                  (std::function<double(int64_t)>));  //!< 角度变化预测函数
  DEFINE_PROPERTY(RecommendedTicks, public, public,
                  (std::vector<int64_t>));  //!< 推荐击打时间戳列表
  DEFINE_PROPERTY(CamToGyro, public, protected,
                  (PoseNode));  //!< 相机坐标系到陀螺仪坐标系的变换位姿
  DEFINE_PROPERTY(LastUpdateTick, protected, protected, (int64_t));  //!< 上一次更新时间戳
  DEFINE_PROPERTY_WITH_INIT(VanishNum, protected, protected, (size_t),
                            0);  //!< 掉帧计数器
  DEFINE_PROPERTY(LastValidPoseGyro, public, protected,
                  (PoseNode));  //!< 最后一次有效观测位姿（陀螺仪坐标系）
  std::shared_ptr<RuneFilterStrategy> _filter =
      nullptr;  //!< 滤波策略，用于对PNP数据进行滤波
 ...
 public:
  ...
  virtual void drawFeature(cv::Mat& image,
                           const DrawConfig_cptr& config = nullptr) const override;
 ...
 private:
  ...
  void updatePnpData(const PoseNode& rune_to_gyro,
                     const GyroData& gyro_data);  //!< 更新PNP数据
  void clearPreviousData();                       //!< 清除上一帧数据
  bool correctRoll(const PoseNode& raw_cam_pnp_data,
                   PoseNode& corrected_cam_pnp_data);  //!< 修正roll角
  PoseNode _correntRoll_last_cam_pnp_data{};           //!< 上一帧相机PNP数据
  bool _correntRoll_last_cam_pnp_data_flag = false;    //!< 数据有效标志
  static bool checkExtremePose(const PoseNode& rune_to_cam, const PoseNode& rune_to_gyro,
