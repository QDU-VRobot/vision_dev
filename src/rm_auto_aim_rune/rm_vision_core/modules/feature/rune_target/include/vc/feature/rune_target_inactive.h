...
class RuneTargetInactive : public RuneTarget
{
 ...
 public:
   ...
   */
  RuneTargetInactive(const Contour_cptr contour, const std::vector<cv::Point2f> corners,
                     std::vector<RuneTargetGap> gaps);
 ...
 protected:
  ...
  static Ptr make_feature(const std::vector<Contour_cptr>& contours,
                          const std::vector<cv::Vec4i>& hierarchy, size_t idx,
 ...
 protected:
  ...
  DEFINE_PROPERTY(
      Gaps, public, protected,
      (std::vector<
          RuneTargetGap>));  //!< 缺口（未排序），排序顺序：左上、右上、右下、左下
  ...
  DEFINE_PROPERTY_WITH_INIT(GapSortedFlag, public, protected, (bool),
                            false);  //!< 缺口是否已排序
  DEFINE_PROPERTY_WITH_INIT(GapCorners, public, protected, (std::vector<cv::Point2f>),
                            {});  //!< 缺口角点占位
...
using RuneTargetInactive_cptr =
    std::shared_ptr<const RuneTargetInactive>;  //!< 只读智能指针
