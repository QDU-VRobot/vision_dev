...
class RuneTarget : public FeatureNode
{
 ...
 public:
  ...
  /**
   * @brief 构造函数
   * @param contour 智能轮廓对象（std::shared_ptr<const
   * ContourWrapper<int>>），用于标定靶心形状
   ...
   * @param p_feature 输入的 const FeatureNode 智能指针（std::shared_ptr<const
   * FeatureNode>）
   * @return 转换后的 const RuneTarget 智能指针（std::shared_ptr<const
   * RuneTarget>），可能为 nullptr
   */
  static inline const std::shared_ptr<const RuneTarget> cast(FeatureNode_cptr p_feature)
  {
    return std::dynamic_pointer_cast<const RuneTarget>(p_feature);
  }
   ...
   *
   * @param[out] targets
   * 返回找到的未激活靶心集合（std::vector<std::shared_ptr<FeatureNode>>）
   * @param[in] contours 输入轮廓集合（std::vector<std::shared_ptr<const
   * ContourWrapper<int>>>）
   * @param[in] hierarchy 轮廓层级信息（OpenCV findContours 输出 std::vector<cv::Vec4i>）
   * @param[in] mask 可跳过的轮廓下标集合（std::unordered_set<size_t>）
   * @param[out] used_contour_idxs
   * 输出使用过的轮廓下标集合（std::unordered_map<std::shared_ptr<const FeatureNode>,
   * std::unordered_set<size_t>>）
   */
  static void find_inactive_targets(
      std::vector<FeatureNode_ptr>& targets, const std::vector<Contour_cptr>& contours,
      const std::vector<cv::Vec4i>& hierarchy, const std::unordered_set<size_t>& mask,
      std::unordered_map<FeatureNode_cptr, std::unordered_set<size_t>>&
          used_contour_idxs);
   ...
   *
   * @param[out] targets
   * 返回找到的激活靶心集合（std::vector<std::shared_ptr<FeatureNode>>）
   * @param[in] contours 输入轮廓集合（std::vector<std::shared_ptr<const
   * ContourWrapper<int>>>）
   * @param[in] hierarchy 轮廓层级信息（OpenCV findContours 输出 std::vector<cv::Vec4i>）
   * @param[in] mask 可跳过的轮廓下标集合（std::unordered_set<size_t>）
   ...
   * std::unordered_set<size_t>>）
   */
  static void find_active_targets(
      std::vector<FeatureNode_ptr>& targets, const std::vector<Contour_cptr>& contours,
      const std::vector<cv::Vec4i>& hierarchy, const std::unordered_set<size_t>& mask,
      std::unordered_map<FeatureNode_cptr, std::unordered_set<size_t>>&
          used_contour_idxs);
