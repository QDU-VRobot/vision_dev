...
RuneTargetInactive::RuneTargetInactive(const Contour_cptr contour,
...
{
  if (hierarchy[idx][3] != -1) return false;
  if (hierarchy[idx][2] == -1) return false;
  int child_num = 1;
...
inline bool isContourInactiveTarget(const vector<Contour_cptr>& contours,
                                    const int& outer_idx,
                                    const unordered_set<size_t>& sub_idxs)
{
  if (contours[outer_idx]->points().size() < 6) return false;
  float contour_area = contours[outer_idx]->area();
  if (contour_area < rune_target_param.INACTIVE_MIN_AREA) return false;
  // 利用长宽比、面积比、周长比进行筛选
...
inline Matx33f getEllipseCorrectionMat(const RotatedRect& ellipse, float& radius)
{
  ...
  Matx33f R(temp_R.at<double>(0, 0), temp_R.at<double>(0, 1), temp_R.at<double>(0, 2),
            temp_R.at<double>(1, 0), temp_R.at<double>(1, 1), temp_R.at<double>(1, 2), 0,
            0, 1);
  Matx33f R_inv = R.inv();
...
inline unordered_set<size_t> getAllGapsIdx(const vector<Contour_cptr>& contours,
                                           ...
                                           const unordered_set<size_t>& all_sub_idx)
{
  ...
  {
    if (hierarchy[sub_idx][3] == -1) pending_idx.erase(sub_idx);
    if (hierarchy[sub_idx][3] != outer_idx) pending_idx.erase(sub_idx);
    if (contours[sub_idx]->points().size() < 10) pending_idx.erase(sub_idx);
  }
  auto outer_fit_ellipse = contours[outer_idx]->fittedEllipse();
  Point2f outer_center = outer_fit_ellipse.center;
  for (auto& sub_idx : all_sub_idx)
  {
    if (pending_idx.find(sub_idx) == pending_idx.end()) continue;
  ...
  for (auto& sub_idx : pending_idx) area_map[sub_idx] = contours[sub_idx]->area();
...
// 获取轮廓最左和最右点的下标
inline tuple<int, int> getLeftRightIdx(const vector<Point2f>& contour,
...
// 创建缺口
inline bool make_gap(const Contour_cptr& contour, const RotatedRect& outer_ellipse,
                     RuneTargetGap& result_gap)
{
  ...
  Point2f left_corner =
      getUnitVector(correction_contour[left_idx] - outer_ellipse.center) *
          rune_target_param.GAP_CIRCLE_RADIUS_RATIO * correction_radius +
      outer_ellipse.center;
  Point2f right_corner =
      getUnitVector(correction_contour[right_idx] - outer_ellipse.center) *
          rune_target_param.GAP_CIRCLE_RADIUS_RATIO * correction_radius +
      outer_ellipse.center;
  Point2f gap_center = getUnitVector(correction_ellipse.center - outer_ellipse.center) *
                           rune_target_param.GAP_CIRCLE_RADIUS_RATIO * correction_radius +
                       outer_ellipse.center;
  float delta_angle = getVectorMinAngle(left_corner - outer_ellipse.center,
                                        right_corner - outer_ellipse.center, DEG);
  if (delta_angle > 100 || delta_angle < 60) return false;

  // 获取缺口方向
  Point2f gap_direction = getUnitVector(right_corner - left_corner),
          center_to_gap = getUnitVector(gap_center - outer_ellipse.center);
...
inline bool filterGaps(vector<RuneTargetGap>& filter_gaps,
...
inline bool findGaps(vector<RuneTargetGap>& gaps, const vector<Contour_cptr>& contours,
                     const vector<Vec4i>& hierarchy, size_t outer_idx,
                     const unordered_set<size_t>& all_sub_idx)
{
  unordered_set<size_t> pending_idx =
      getAllGapsIdx(contours, hierarchy, outer_idx, all_sub_idx);
  if (pending_idx.empty()) return false;
  auto outer_ellipse = contours[outer_idx]->fittedEllipse();
  vector<RuneTargetGap> all_gaps;
  for (auto& gap_idx : pending_idx)
  {
    RuneTargetGap gap;
    if (!make_gap(contours[gap_idx], outer_ellipse, gap)) continue;
    all_gaps.push_back(gap);
  }
  vector<RuneTargetGap> filter_gaps;
  if (!filterGaps(filter_gaps, all_gaps)) return false;
  if (filter_gaps.empty() || filter_gaps.size() > 4) return false;
  gaps = filter_gaps;
  return true;
}

auto RuneTargetInactive::make_feature(const vector<Contour_cptr>& contours,
                                      const vector<Vec4i>& hierarchy, size_t idx,
                                      unordered_set<size_t>& used_contour_idxs)
    -> RuneTargetInactive_ptr
{
  ...
  if (!isContourInactiveTarget(contours, idx, sub_contours_idx)) return nullptr;
  vector<RuneTargetGap> gaps;
  if (!findGaps(gaps, contours, hierarchy, idx, sub_contours_idx)) return nullptr;
...
void RuneTargetInactive::find(
    vector<FeatureNode_ptr>& targets, const vector<Contour_cptr>& contours,
...
{
  if (isnan(rotate_center.x) || isnan(rotate_center.y)) return false;
  if (getActiveFlag())
  {
  }
  else
  {
    if (!correctCorners(rotate_center)) return false;
...
bool RuneTargetInactive::sortedGap(const Point2f& rune_center)
{
  ...
  auto& gaps = getGaps();
  if (gaps.empty() || gaps.size() > 4) return false;
  ...
  for (auto& gap : gaps)
  {
    ...
    float dot = center_to_RuneCenter.x * center_to_gap.x +
                center_to_RuneCenter.y * center_to_gap.y;
...
bool RuneTargetInactive::calcGapCorners(const Point2f& rune_center)
{
  ...
  {
    if (gap_corner != VACANCY_POINT) target_corners.push_back(gap_corner);
  }
  return true;
}
...
bool RuneTargetInactive::correctCorners(const Point2f& rune_center)
{
  ...
  if (!getGapSortedFlag() && !sortedGap(rune_center)) return false;
  if (!calcGapCorners(rune_center)) return false;
  return true;
}
...
bool RuneTargetInactive::correctDirection()
{
  ...
  if (!getGapSortedFlag()) return false;
  ...
  if (getRightTopGap() && getRightBottomGap())
    direction_right =
        getUnitVector(getRightBottomGap()->center - getRightTopGap()->center);
  if (direction_left == Point2f(0, 0) && direction_right == Point2f(0, 0)) return false;
    ...
    -> tuple<vector<Point2f>, vector<Point3f>, vector<float>>
{
  ...
  auto& gap_corners = getGapCorners();
  if (gap_corners.size() != 8) VC_THROW_ERROR("The gap corners size is not 8.");
...
void RuneTargetInactive::drawFeature(Mat& image, const DrawConfig_cptr& config) const
{
  auto draw_circle = [&]() -> bool
  {
    ...
    if (!image_info.isSetCorners()) return false;
    ...
    if (radius > 30) circle(image, center, radius * 0.5f, circle_color, circle_thickness);
    if (radius > 50)
  ...
  {
    const auto& image_info = getImageCache();
    if (!image_info.isSetContours()) return false;
    const auto& contours = image_info.getContours();
    if (contours.empty() || contours.front()->points().size() < 6) return false;
...
RuneTargetInactive_ptr RuneTargetInactive::make_feature(const PoseNode& target_to_cam)
{
  ...
  for (const auto& corner : rune_target_param.GAP_3D) corners_3d.emplace_back(corner);
  vector<Point2f> corners_2d;
  projectPoints(corners_3d, target_to_cam.rvec(), target_to_cam.tvec(),
                camera_param.cameraMatrix, camera_param.distCoeff, corners_2d);
  vector<Point2f> temp_rune_center;
  projectPoints(vector<Point3f>{Point3f(0, 0, 0)}, target_to_cam.rvec(),
                target_to_cam.tvec(), camera_param.cameraMatrix, camera_param.distCoeff,
                temp_rune_center);
