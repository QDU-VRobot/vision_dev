...
RuneTargetActive::RuneTargetActive(const Contour_cptr contour,
...
inline bool isHierarchyActiveTarget(const vector<Contour_cptr>& contours,
    ...
    unordered_map<FeatureNode_cptr, unordered_set<size_t>>& used_contour_idxs)
{
  for (size_t i = 0; i < contours.size(); i++)
  {
    ...
    if (isHierarchyActiveTarget(contours, hierarchy, i))
    {
      ...
      auto p_target =
          RuneTargetActive::make_feature(contours, hierarchy, i, temp_used_contour_idxs);
      if (p_target)
      {
...
{
  return make_tuple(vector<Point2f>{getImageCache().getCenter()},
                    vector<Point3f>{Point3f(0, 0, 0)}, vector<float>{1.0});
}

inline bool checkEllipse(const Contour_cptr& contour)
{
  if (contour->points().size() < 6) return false;
  ...
  if (hull_area_ratio > rune_target_param.ACTIVE_MAX_CONVEX_AREA_RATIO) return false;
  ...
  if (hull_perimeter_ratio > rune_target_param.ACTIVE_MAX_CONVEX_PERI_RATIO) return false;
  return true;
}

inline bool checkConcentricity(const vector<Contour_cptr>& contours,
                               const vector<Vec4i>& hierarchy,
                               const vector<int>& all_sub_idx, size_t idx,
                               double contour_area)
{
  if (all_sub_idx.empty()) return false;
  unordered_map<size_t, float> area_map;
  for (auto sub_idx : all_sub_idx) area_map[sub_idx] = contours[sub_idx]->area();
  auto [max_area_idx, max_sub_area] =
      *max_element(area_map.begin(), area_map.end(), [](const auto& lhs, const auto& rhs)
                   { return lhs.second < rhs.second; });
  if (max_sub_area > contour_area) VC_THROW_ERROR("sub_contour_area > outer_area");
  if (max_sub_area / contour_area < rune_target_param.ACTIVE_MIN_AREA_RATIO_SUB)
    return false;
  return checkEllipse(contours[max_area_idx]);
}

inline bool checkTenRing(const vector<Contour_cptr>& contours,
                         const vector<Vec4i>& hierarchy, const vector<int>& all_sub_idx,
                         size_t idx, double contour_area)
{
  float total_area = 0;
  for (auto sub_idx : all_sub_idx) total_area += contours[sub_idx]->area();
  return total_area / contour_area <=
         rune_target_param.ACTIVE_MAX_AREA_RATIO_SUB_TEN_RING;
}

RuneTargetActive_ptr RuneTargetActive::make_feature(
    const vector<Contour_cptr>& contours, const vector<Vec4i>& hierarchy, size_t idx,
    unordered_set<size_t>& used_contour_idxs)
{
  const auto& contour_outer = contours[idx];
  if (contour_outer->points().size() < 6) return nullptr;
  float contour_area = contour_outer->area();
  if (!checkEllipse(contour_outer)) return nullptr;
  vector<int> all_sub_idx;
  getAllSubContoursIdx(hierarchy, idx, all_sub_idx);
  if (contour_area < 100) return nullptr;
  if (!checkConcentricity(contours, hierarchy, all_sub_idx, idx, contour_area) &&
      !checkTenRing(contours, hierarchy, all_sub_idx, idx, contour_area))
    return nullptr;
...
void RuneTargetActive::drawFeature(Mat& image, const DrawConfig_cptr& config) const
{
  ...
  auto draw_circle = [&]()
  {
    if (!image_info.isSetCorners() || !image_info.isSetCenter()) return false;
    ...
    auto radius = image_info.isSetHeight() && image_info.isSetWidth()
                      ...
                      : rune_target_draw_param.active.point_radius;
...
{
  vector<Point3f> corners_3d{{0, -rune_target_param.RADIUS, 0},
