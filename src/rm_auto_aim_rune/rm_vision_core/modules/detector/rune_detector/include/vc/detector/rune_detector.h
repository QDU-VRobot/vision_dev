...
using RuneFeatureComboConst =
    std::tuple<FeatureNode_cptr, FeatureNode_cptr, FeatureNode_cptr>;
enum class RuneType : uint8_t;  // forward (defined in rune_combo)
class RuneDetector : public Detector
{
 ...
 public:
  ...
  /**
   * @brief Raw detection for ROS2: feature detection + group PnP only, WITHOUT any
   * tracking/filtering.
   *
   * This is used to decouple detector and tracker in ROS2 architecture.
   */
  struct RawDetectionResult
  {
    PoseNode group_to_cam;
    std::array<uint8_t, 5> rune_type;
  };

  bool detect(const cv::Mat& image, PixChannel target_color, uint8_t threshold,
              RawDetectionResult& out);
 ...
 private:
   ...
   */
  static void binary(const cv::Mat& src, cv::Mat& bin, PixChannel target_color,
                     uint8_t threshold);
   ...
   * @param[out] matched_features 配对好的特征 (靶心、中心、扇叶),未找出的特征将会用
   * nullptr 补齐
   */
  static bool findFeatures(cv::Mat src, std::vector<FeatureNode_cptr>& features,
   ...
   */
  static void extractRuneFeatures(std::vector<FeatureNode_ptr>& targets_inactive,
                                  std::vector<FeatureNode_ptr>& targets_active,
                                  std::vector<FeatureNode_ptr>& fans_inactive,
                                  std::vector<FeatureNode_ptr>& fans_active,
                                  std::vector<FeatureNode_ptr>& centers,
   ...
   */
  bool match(const std::vector<FeatureNode_ptr>& combos,
             std::vector<FeatureNode_ptr>& trackers, bool is_vanish_update);
   ...
   */
  static bool filterActiveTarget(std::vector<FeatureNode_ptr>& active_targets,
   ...
   *
   */
  static bool getRotateCenter(const std::vector<FeatureNode_ptr>& targets,
                              const std::vector<FeatureNode_ptr>& fans,
   ...
   * @return RuneC_ptr
   */
  FeatureNode_ptr getBestCenter(const std::vector<FeatureNode_ptr>& p_targets,
   ...
   */
  bool getPnpData(PoseNode& camera_pnp_data, const FeatureNode_ptr& group,
