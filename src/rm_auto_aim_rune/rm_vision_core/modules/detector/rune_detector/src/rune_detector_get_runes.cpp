...
inline bool setTempRunePnpData(vector<PoseNode>& pnp_datas,
                               const PoseNode& camera_pnp_data)
{
  if (pnp_datas.size() != 5) VC_THROW_ERROR("pnp_datas size != 5");
...
inline bool setTempRuneType(
    vector<RuneType>& types, const vector<PoseNode>& pnp_datas,
    const vector<tuple<FeatureNode_cptr, FeatureNode_cptr, FeatureNode_cptr>>&
        features_visible,
    const FeatureNode_ptr& group)
{
  if (types.size() != 5 || pnp_datas.size() != 5) VC_THROW_ERROR("size != 5");
  unordered_set<size_t> pending = {0, 1, 2, 3, 4};
  auto getProj = [&](const Matx31f& src, const PoseNode& p)
  {
    vector<Point2f> proj;
    projectPoints(src, p.rvec(), p.tvec(), camera_param.cameraMatrix,
  ...
  for (auto& grp : features_visible)
  {
    ...
    RuneType type =
        fan ? (RuneFan::cast(fan)->getActiveFlag() ? RuneType::STRUCK
                                                   : RuneType::PENDING_STRUCK)
            : (RuneTarget::cast(target)->getActiveFlag() ? RuneType::STRUCK
                                                         : RuneType::PENDING_STRUCK);
    int idx = -1;
    ...
    for (auto& n : pending)
    {
      ...
      Point2f p2 = target ? getProj(rune_target_param.TRANSLATION, pnp_datas[n])
                          : getProj(RuneFan::cast(fan)->getActiveFlag()
  ...
  for (auto& idx : pending) types[idx] = RuneType::UNSTRUCK;
  return true;
}

inline bool setAllfeatures(
    const vector<PoseNode>& pnp_datas, const vector<RuneType>& types,
    vector<tuple<FeatureNode_ptr, FeatureNode_ptr, FeatureNode_ptr>>& rune_features)
{
  if (pnp_datas.size() != 5 || types.size() != 5) VC_THROW_ERROR("size != 5");
  ...
  for (size_t i = 0; i < 5; i++)
  {
    ...
    FeatureNode_ptr target =
        (types[i] == RuneType::STRUCK)
            ? static_cast<FeatureNode_ptr>(
                  RuneTargetActive::make_feature(target_to_rune + rune_to_cam))
            : static_cast<FeatureNode_ptr>(
                  RuneTargetInactive::make_feature(target_to_rune + rune_to_cam));

    // 中心
    PoseNode center_to_cam =
        PoseNode(rune_center_param.ROTATION, rune_center_param.TRANSLATION) + rune_to_cam;
    FeatureNode_ptr center = RuneCenter::make_feature(center_to_cam);

    // 扇叶
    PoseNode fan_to_rune =
        (types[i] == RuneType::STRUCK)
            ? PoseNode(rune_fan_param.ACTIVE_ROTATION, rune_fan_param.ACTIVE_TRANSLATION)
            : PoseNode(rune_fan_param.INACTIVE_ROTATION,
                       rune_fan_param.INACTIVE_TRANSLATION);
    FeatureNode_ptr fan =
        RuneFan::make_feature(fan_to_rune + rune_to_cam, types[i] == RuneType::STRUCK);

    rune_features[i] = {target, center, fan};
    if (!target || !center || !fan) return false;
  }
  return true;
}
...
{
  if (!group || features_visible.empty()) return false;
  ...
  if (!setTempRunePnpData(pnp_datas, camera_pnp_data)) return false;
  if (!setTempRuneType(pnp_types, pnp_datas, features_visible, group)) return false;
  if (!setAllfeatures(pnp_datas, pnp_types, pnp_runes)) return false;

  current_combos.clear();
  for (size_t i = 0; i < 5; i++)
  {
    auto [target, center, fan] = pnp_runes[i];
    if (!target || !center || !fan) continue;
    auto rune =
        RuneCombo::make_feature(pnp_datas[i], pnp_types[i], getGyroData(), getTick());
    if (rune) current_combos.push_back(dynamic_pointer_cast<RuneCombo>(rune));
  }
  return true;
}
