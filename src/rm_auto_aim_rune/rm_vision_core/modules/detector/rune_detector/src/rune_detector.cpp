 ...
 */
static inline void setFeatures(const std::vector<RuneFeatureComboConst>& matched_features,
                               std::vector<FeatureNode_cptr>& features)
{
  // 更新特征组
  for (const auto& [target, center, fan] : matched_features)
  {
    if (target) features.emplace_back(target);
    if (center) features.emplace_back(center);
    if (fan) features.emplace_back(fan);
  }
}
 ...
 * @brief 设置序列组的基础属性
 */
inline static void setBaseProperties(std::vector<FeatureNode_ptr>& groups,
...
bool RuneDetector::detect(const cv::Mat& image, PixChannel target_color,
                          uint8_t threshold, RawDetectionResult& out)
{
  // 1) Binary
  cv::Mat bin;
  binary(image, bin, target_color, threshold);

  // 2) Find features + matched feature tuples
  std::vector<FeatureNode_cptr> current_features;
  std::vector<RuneFeatureCombo> matched_features;
  if (!findFeatures(bin, current_features, matched_features)) return false;

  // 3) Build group pose via PnP
  FeatureNode_ptr group = RuneGroup::make_feature();
  PoseNode camera_pnp_data;
  if (!getPnpData(camera_pnp_data, group, to_const(matched_features))) return false;

  // 4) Compute per-sector rune type using existing logic (getRunes)
  std::vector<FeatureNode_ptr> combos;
  if (!getRunes(combos, group, to_const(matched_features), camera_pnp_data)) return false;
  if (combos.size() != 5)
  {
    return false;
  }

  out.group_to_cam = camera_pnp_data;
  for (size_t i = 0; i < 5; ++i)
  {
    auto combo = std::dynamic_pointer_cast<RuneCombo>(combos[i]);
    if (!combo)
    {
      out.rune_type[i] = 0;
    }
    ...
    {
      out.rune_type[i] = static_cast<uint8_t>(combo->getRuneType());
    }
  }
  return true;
}
...
void RuneDetector::detect(DetectorInput& input, DetectorOutput& output)
{
  ...
  // 初始化存储信息
  if (groups.empty()) groups.emplace_back(RuneGroup::make_feature());
  ...
  auto updateRuneGroup = [&]() -> bool
  {
    ...
    // 更新序列组
    if (!rune_group->update(runeGroup_to_cam, gyro_data, tick)) return false;

    // 尝试获取所有的神符组合体
    if (!getRunes(current_combos, rune_group, to_const(matched_features),
  ...
  {
    // 掉帧状态处理
    if (!rune_group->visibilityProcess(false)) return false;

    PoseNode runeGroup_to_cam;
    if (!rune_group->getCamPnpDataFromPast(runeGroup_to_cam)) return false;

    // 更新序列组
    if (!rune_group->update(runeGroup_to_cam, gyro_data, tick)) return false;

    // 尝试获取所有的神符组合体
    if (!getRunes(current_combos, rune_group, rune_group->getLastFrameFeatures(),
                  rune_group->getPoseCache().getPoseNodes()[CoordFrame::CAMERA]))
      return false;
  ...
  if (matched_features.size() > 1) rune_group->drawFeature(img_show);
