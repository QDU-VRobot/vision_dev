...
{
  if (!feature) return Point2f(0.f, 0.f);
  return feature->getImageCache().getCenter();
}

void RuneDetector::binary(const cv::Mat& src, cv::Mat& bin, PixChannel target_color,
                          uint8_t threshold)
{
  if (src.type() != CV_8UC3)
  {
...
bool RuneDetector::filterInactiveTarget(std::vector<FeatureNode_ptr>& inactive_targets)
{
  ...
  {
    // 选择面积最大的未激活靶心
    auto max_target = *max_element(
        inactive_targets.begin(), inactive_targets.end(),
        [](const FeatureNode_ptr& t1, const FeatureNode_ptr& t2)
        {
          return t1->getImageCache().getContours().front()->minAreaRect().size.area() <
                 t2->getImageCache().getContours().front()->minAreaRect().size.area();
        });
...
bool RuneDetector::filterActiveTarget(std::vector<FeatureNode_ptr>& active_targets,
                                      std::vector<FeatureNode_ptr>& inactive_targets)
{
  if (active_targets.size() > 4)
  {
    Point2f ave_center{};
    for (auto& target : active_targets) ave_center += target->getImageCache().getCenter();
    ave_center /= static_cast<float>(active_targets.size());

    // 选择距离中心最近的4个激活靶心
    sort(active_targets.begin(), active_targets.end(),
         [&ave_center](const FeatureNode_ptr& t1, const FeatureNode_ptr& t2)
         {
           return getDist(t1->getImageCache().getCenter(), ave_center) <
                  getDist(t2->getImageCache().getCenter(), ave_center);
         });
...
{
  if (inactive_fans.empty()) return false;
  if (inactive_targets.empty() && active_targets.empty() && active_fans.empty())
  {
    // 当没有参考特征时，采用面积最大的未激活扇叶
    auto max_fan =
        *max_element(inactive_fans.begin(), inactive_fans.end(),
                     [](const FeatureNode_ptr& f1, const FeatureNode_ptr& f2)
  ...
  for (auto& inactive_fan : inactive_fans) fan_accuracy_map[inactive_fan] = 1.0;

  // 利用神符靶心筛选
  if (inactive_targets.size() + active_targets.size() >= 3)
  {
    vector<Point> target_centers{};
    for (auto& p_target : inactive_targets) target_centers.push_back(getCenter(p_target));
    for (auto& p_target : active_targets) target_centers.push_back(getCenter(p_target));
    ...
    for (auto& inactive_fan : inactive_fans)
    {
      ...
      if (distance > radius) fan_accuracy_map[inactive_fan] *= (1 - target_accuracy);
    }
  }
  ...
  if (active_fans.size() >= 2)
  {
    ...
    for (size_t i = 0; i < active_fans.size(); i++)
    {
      for (size_t j = i + 1; j < active_fans.size(); j++)
      {
        ...
        Vec4f line1{getCenter(fan1).x, getCenter(fan1).y,
                    getCenter(fan1).x + fan1->getImageCache().getDirection().x,
                    getCenter(fan1).y + fan1->getImageCache().getDirection().y};
        Vec4f line2{getCenter(fan2).x, getCenter(fan2).y,
                    getCenter(fan2).x + fan2->getImageCache().getDirection().x,
                    getCenter(fan2).y + fan2->getImageCache().getDirection().y};
        Point2f cross_point = getCrossPoint(line1, line2);
        temp_centers.emplace_back(cross_point);
      }
    }
    Point2f ave_center =
        std::accumulate(temp_centers.begin(), temp_centers.end(), Point2f(0, 0)) /
        static_cast<double>(temp_centers.size());  // 平均中心
    // 设置已激活扇叶的正确率
    ...
    for (auto& inactive_fan : inactive_fans)
    {
      Vec4f direction_line{inactive_fan->getImageCache().getDirection().x,
                           inactive_fan->getImageCache().getDirection().y,
                           getCenter(inactive_fan).x, getCenter(inactive_fan).y};
      if (getDist(direction_line, ave_center) >
          max(inactive_fan->getImageCache().getHeight(),
              inactive_fan->getImageCache().getWidth()))
  ...
  if (inactive_fans.size() >= 2)
  {
    for (size_t i = 0; i < inactive_fans.size(); i++)
    {
      for (size_t j = i + 1; j < inactive_fans.size(); j++)
      {
        ...
        if (pointPolygonTest(points_1, getCenter(fan2), false) > 0 ||
            pointPolygonTest(points_2, getCenter(fan1), false) > 0)
        {
          if (fan1->getImageCache().getContours().front()->area() >
              fan2->getImageCache().getContours().front()->area())
  ...
  auto [min_distance_fan, distance] = *min_element(
      fan_distance.begin(), fan_distance.end(),
  ...
  if (max_dis >
      max(best_fan->getImageCache().getHeight(), best_fan->getImageCache().getWidth()) *
          rune_fan_param.INACTIVE_MAX_DISTANCE_RATIO)
  {
    inactive_fans.clear();
    return false;
  }
                                     ...
                                     const vector<FeatureNode_ptr>& active_fans)
{
  ...
  bool result =
      filterInactiveFanImpl(inactive_fans, inactive_targets, active_targets, active_fans);

  for (auto& used_fan : inactive_fans)
  {
    if (erased_fan.find(used_fan) == erased_fan.end()) continue;
...
struct RuneDetectorFilterCenterParam
{
  ...
  YML_INIT(RuneDetectorFilterCenterParam,
           YML_ADD_PARAM(MAX_TARGET_MID_LINE_DISTANCE_RATIO);
           YML_ADD_PARAM(MAX_TARGET_DISTANCE_RATIO);
           YML_ADD_PARAM(MIN_TARGET_DISTANCE_RATIO); YML_ADD_PARAM(MAX_FAN_ANGLE);
                                ...
                                const std::vector<FeatureNode_ptr>& active_fans)
{
  ...
  if (inactive_targets.empty() && active_targets.empty() && inactive_fans.empty() &&
      active_fans.empty())
  {
    // 当没有参考特征时，采用面积最大的神符中心
    auto max_center =
        *max_element(centers.begin(), centers.end(),
                     [](const FeatureNode_ptr& c1, const FeatureNode_ptr& c2)
                     {
                       return c1->getImageCache().getContours().front()->area() <
                              c2->getImageCache().getContours().front()->area();
                     });
    return false;
  }
  if (centers.size() > 30)
  ...
  for (auto& center : centers) center_accuracy[center] = 1;  // 初始化为正确概率为 1.0

  // 特殊筛选（1）
  if (inactive_targets.size() == 1 && inactive_fans.size() == 1)
  {
    auto target_rect =
        inactive_targets.front()->getImageCache().getContours().front()->minAreaRect();
    auto fan_rect =
        inactive_fans.front()->getImageCache().getContours().front()->minAreaRect();
  ...
  for (size_t i = 0; i < all_targets.size(); i++)
  {
    ...
    for (size_t j = i + 1; j < all_targets.size(); j++)
    {
      ...
      Vec4f mid_line = {vertical_direction[0], vertical_direction[1], mid_point.x,
                        mid_point.y};
      double target_accuracy = 1.0;
      target_accuracy *= all_targets[i]->getActiveFlag()
                             ? rune_detector_param.ACTIVE_TARGET_ACCURACY
                             : rune_detector_param.INACTIVE_TARGET_ACCURACY;
      target_accuracy *= all_targets[j]->getActiveFlag()
                             ? rune_detector_param.ACTIVE_TARGET_ACCURACY
                             : rune_detector_param.INACTIVE_TARGET_ACCURACY;
      for (auto& center : centers)
      {
        float distance = getDist(mid_line, getCenter(center));
        float distance_ratio = distance / max(center->getImageCache().getWidth(),
                                              center->getImageCache().getHeight());
        if (distance_ratio >
            rune_detector_filter_center_param.MAX_TARGET_MID_LINE_DISTANCE_RATIO)
        {
    ...
    for (auto& center : centers)
    {
      ...
      float distance_ratio = distance / max(all_targets[i]->getImageCache().getWidth(),
                                            all_targets[i]->getImageCache().getHeight());
      if (distance_ratio > rune_detector_filter_center_param.MAX_TARGET_DISTANCE_RATIO)
      {
        center_accuracy[center] *= (1 - all_targets[i]->getActiveFlag()
                                        ? rune_detector_param.ACTIVE_TARGET_ACCURACY
                                        : rune_detector_param.INACTIVE_TARGET_ACCURACY);
      }
  ...
  for (auto& fan : inactive_fans)
  {
    ...
    for (auto& center : centers)
    {
      ...
      float distance_ratio = distance / max(fan->getImageCache().getWidth(),
                                            fan->getImageCache().getHeight());
  ...
  for (auto& target : inactive_targets)
  {
    ...
    contour_accuracy.push_back(
        {rect_points, rune_detector_param.INACTIVE_TARGET_ACCURACY});
  }
  ...
  centers.erase(
      remove_if(
          centers.begin(), centers.end(), [&](const auto& center)
  ...
  // 选择正确率最高的神符中心
  auto [best_center, accuracy] = *max_element(
      center_accuracy.begin(), center_accuracy.end(),
      [](const auto& lhs, const auto& rhs) { return lhs.second < rhs.second; });
...
bool RuneDetector::filterActiveFanIncomplete(
...
{
  if (fans_Incomplete.empty()) return false;
  ...
  if (!active_targets.empty())
  {
    ...
    {
      auto min_target =
          *min_element(active_targets.begin(), active_targets.end(),
                       [&fan](const FeatureNode_ptr& t1, const FeatureNode_ptr& t2)
                       {
                         return getDist(getCenter(fan), getCenter(t1)) <
                                getDist(getCenter(fan), getCenter(t2));
                       });
      ...
      if (delta_angle > 20) continue;
      if (distance >
          std::max(fan->getImageCache().getWidth(), fan->getImageCache().getHeight()))
        continue;
  ...
  {
    if (filtered_fans.find(fan1) == filtered_fans.end()) continue;
    for (const auto& fan2 : fans_Incomplete)
    {
      if (fan1 == fan2) continue;
      if (filtered_fans.find(fan2) == filtered_fans.end()) continue;

      float distance = getDist(getCenter(fan1), getCenter(fan2));
      float min_threshold =
          (std::max(fan1->getImageCache().getWidth(), fan1->getImageCache().getHeight()) +
           std::max(fan2->getImageCache().getWidth(),
                    fan2->getImageCache().getHeight())) /
          4.0f;
  ...
  fans_Incomplete.insert(fans_Incomplete.end(), filtered_fans.begin(),
                         filtered_fans.end());
                                     ...
                                     std::unordered_set<size_t>& far_contour_idxs)
{
  ...
  if (targets.empty() && fans.empty()) return false;
  ...
  if (max_distance == 0.f) return false;
  ...
  for (size_t i = 0; i < contours.size(); ++i)
  {
    ...
    if (distance > max_distance_threshold) far_contour_idxs.insert(i);
  }

  return true;
}

bool RuneDetector::getRotateCenter(const std::vector<FeatureNode_ptr>& targets,
                                   const std::vector<FeatureNode_ptr>& fans,
                                   cv::Point2f& rotate_center)
{
  ...
  for (const auto& target : targets)
  {
    ...
    {
      cv::Vec4f line{
          getCenter(rune_target).x, getCenter(rune_target).y,
  ...
  if (cross_points.empty()) return false;

  // 获取所有特征的方向射线的交点的平均值
  rotate_center =
      std::accumulate(cross_points.begin(), cross_points.end(), cv::Point2f(0, 0)) /
      static_cast<float>(cross_points.size());

  return true;
}

std::vector<std::tuple<FeatureNode_ptr, FeatureNode_ptr, FeatureNode_ptr>>
RuneDetector::getMatchedFeature(const std::vector<FeatureNode_ptr>& p_targets,
                                ...
                                const std::vector<FeatureNode_ptr>& p_fans)
{
  ...
  for (auto& fan : p_fans)
  {
    ...
    {
      delta_angles[target] = getVectorMinAngle(-1 * fan->getImageCache().getDirection(),
                                               getCenter(target) - getCenter(fan), DEG);
    }
    // 找到最小角度差的靶心
    auto [target, min_angle] = *min_element(delta_angles.begin(), delta_angles.end(),
                                            [](const pair<FeatureNode_ptr, float>& lhs,
                                               const pair<FeatureNode_ptr, float>& rhs)
                                            { return lhs.second < rhs.second; });
    // 如果角度差小于阈值，则认为匹配成功
    if (min_angle < 20)
    {
      if (target == nullptr || fan == nullptr) VC_THROW_ERROR("target or fan is nullptr");
  ...
  {
    if (pending_targets.find(match_target) == pending_targets.end()) continue;
    ...
    for (auto& [target, center, fan] : matched_feature)
    {
      ...
      min_delta_angle =
          std::min(min_delta_angle, getVectorMinAngle(direction_1, direction_2, DEG));
    }
  ...
  {
    if (pending_fans.find(match_fan) == pending_fans.end()) continue;
