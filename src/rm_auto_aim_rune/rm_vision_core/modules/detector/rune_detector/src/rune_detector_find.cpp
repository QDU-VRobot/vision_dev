...
inline bool isMatchError(const std::vector<RuneFeatureCombo>& runes)
{
  ...
  {
    if ((target == nullptr && fan == nullptr) || center == nullptr) return true;

    if (target != nullptr && fan != nullptr)
    {
      if (RuneTarget::cast(target)->getActiveFlag() !=
          RuneFan::cast(fan)->getActiveFlag())
        return true;
    }
  }
  return false;
...
FeatureNode_ptr forceMakeCenterByInactiveFan(
    vector<FeatureNode_ptr>& inactive_fans,
    const vector<FeatureNode_ptr>& inactive_targets)
{
  if (inactive_fans.empty() || inactive_targets.empty()) return nullptr;
  ...
  {
    auto end_arrow_contour =
        RuneFanInactive::getEndArrowContour(fan, to_const(in_targets));
    if (end_arrow_contour == nullptr) continue;
    auto rune_center = RuneCenter::make_feature(end_arrow_contour, {});
    if (rune_center == nullptr) continue;

    rune_centers.push_back(rune_center);
  }
  // 更新 inactive_fans
  inactive_fans.clear();
  for (auto& fan : in_fans) inactive_fans.push_back(RuneFan::cast(fan));
 ...
 *       3. 仅用于临时测试使用
 */
FeatureNode_ptr forceMakeCenter(const vector<FeatureNode_ptr>& inactive_targets,
                                ...
                                const vector<FeatureNode_ptr>& active_fans)
{
  ...
  if (rune_center != nullptr) return rune_center;
  ...
  if (targets.empty() && fans.empty()) return nullptr;
  ...
  if (fans.size() >= 2)
  {
    ...
    for (size_t i = 0; i < fans.size() - 1; i++)
    {
      for (size_t j = i + 1; j < fans.size(); j++)
      {
        ...
        if (intersection == cv::Point2f(std::numeric_limits<float>::quiet_NaN(),
    ...
    if (!fans_intersection.empty())
    {
      ...
      fans_center =
          std::make_unique<Point2f>(sum.x / static_cast<float>(fans_intersection.size()),
                                    sum.y / static_cast<float>(fans_intersection.size()));
    }
  }
  Point2f center{0, 0};
  if (targets_center == nullptr && fans_center == nullptr) return nullptr;
...
{
  {
    ...
    RuneTarget::find_inactive_targets(targets_inactive, contours, hierarchy, continue_idx,
                                      used_idxs);
  ...
  {
    unordered_map<FeatureNode_cptr, unordered_set<size_t>> used_idxs{};
    RuneTarget::find_active_targets(targets_active, contours, hierarchy, continue_idx,
                                    used_idxs);
  ...
  }
  {
    ...
    targets_inactive_temp.insert(targets_inactive_temp.end(), targets_inactive.begin(),
                                 targets_inactive.end());
    RuneFan::find_inactive_fans(fans_inactive, contours, hierarchy, continue_idx,
                                used_idxs, targets_inactive_temp);
...
bool RuneDetector::findFeatures(Mat src, vector<FeatureNode_cptr>& features,
                                std::vector<RuneFeatureCombo>& matched_features)
{
  ...
  // vector<Contour_ptr> contours(contours_temp.begin(), contours_temp.end()); //
  // 轮廓二维向量
  ...
  extractRuneFeatures(targets_inactive, targets_active, fans_inactive, fans_active,
                      centers, contours, hierarchy, continue_idx);
  ...
  {
    if (centers.empty()) return true;  // 神符中心为空、需要强制构造
  ...
  do
  {
    ...
    // 尝试强制构造神符中心
    auto center =
        forceMakeCenter(targets_inactive, targets_active, fans_inactive, fans_active);
    if (center)
    {
      centers.clear();
  ...
  RuneFanInactive::correctDirection(rune_inactive_fan,
                                    getCenter(rune_center));  // 未激活扇叶的方向矫正
  ...
  rune_targets.insert(rune_targets.end(), targets_inactive.begin(),
                      targets_inactive.end());
  ...
  RuneDetector::filterFarContours(contours, rune_targets, rune_center, rune_fans,
                                  continue_idx, far_contour_idxs);  // 过滤距离过远的轮廓
  ...
  filterActiveFanIncomplete(fans_active_incomplete, targets_inactive, targets_active,
                            fans_inactive, fans_active,
                            rune_center);  // 筛选未完整的已激活扇叶
  rune_fans.insert(rune_fans.end(), fans_active_incomplete.begin(),
                   fans_active_incomplete.end());
  ...
  {
    if (target != nullptr) features.emplace_back(target);
    if (center != nullptr) features.emplace_back(center);
    if (fan != nullptr) features.emplace_back(fan);
  }
  return true;
}
