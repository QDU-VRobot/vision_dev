...
inline vector<float> getRuneDeviation(const std::vector<RuneFeatureComboConst>& runes)
{
  ...
  {
    if ((target && !RuneTarget::cast(target)->getActiveFlag()) ||
        (fan && !RuneFan::cast(fan)->getActiveFlag()))
    {
  ...
  for (auto& [features, angle] : angle_map) angle = normalizeAngle(angle);

  // 按角度对神符进行排序.
  sort(temp_runes.begin(), temp_runes.end(),
       [&](const RuneFeatureComboConst& rune_1, const RuneFeatureComboConst& rune_2)
       { return angle_map[rune_1] < angle_map[rune_2]; });

  vector<float> reference_angles{};
  for (size_t i = 0; i < 5; i++) reference_angles.push_back(72 * static_cast<int>(i));
  ...
  auto [rotate_angle, min_delta] = *min_element(
      rotate_deltas.begin(), rotate_deltas.end(),
  ...
  for (auto& feature_group : temp_runes)
  {
    if (pending_num.empty()) break;
    float angle = angle_map[feature_group];
    int min_delta_idx = -1;
    float min_delta = 1e5;
    for (size_t i = 0; i < 5; i++)
    {
      if (pending_num.find(static_cast<int>(i)) == pending_num.end()) continue;
  ...
  if (rune_idx.size() > 5) VC_THROW_ERROR("The rune_idx size is greater than 5");

  // 将 最小编号设置为 0 ，其余编号依次递增
  int min_idx = 1e5;
  for (auto& idx : rune_idx) min_idx = min(min_idx, idx.second);
  for (auto& [features, idx] : rune_idx) idx -= min_idx;
 ...
 */
inline void rotatePoints(const vector<Point3f>& raw_points, float angle,
                         vector<Point3f>& rotated_points)
{
  rotated_points.clear();
  // 计算旋转矩阵
  Matx33f rotate_mat = {cos(deg2rad(angle)),
                        -sin(deg2rad(angle)),
                        0,
                        sin(deg2rad(angle)),
                        cos(deg2rad(angle)),
  ...
  for (const auto& point : raw_points)
  {
    ...
    rotated_points.emplace_back(rotated_point_3d(0), rotated_point_3d(1),
                                rotated_point_3d(2));
  }
}
                         ...
                         std::vector<float>& point_weights)
{
  ...
  map<tuple<FeatureNode_cptr, FeatureNode_cptr, FeatureNode_cptr>, float>
      rune_deviation_angle{};  // 特征组对应的偏移角度
  ...
  for (auto& feature_group : runes)
  {
    ...
    if (target != nullptr)
    {
      ...
      if (target->getActiveFlag())
      {
        for (int i = 0; i < 5; i++)
        {
          ...
          rune_points_weight.insert(rune_points_weight.end(), points_weight.begin(),
                                    points_weight.end());
        }
      }
      else
    ...
    Point3f rotCenter_to_target(rune_center_param.TRANSLATION(0),
                                rune_center_param.TRANSLATION(1), 0);
    // 顺时针旋转
    Matx33f rotate_mat = {cos(deg2rad(deviation_angle)),
                          -sin(deg2rad(deviation_angle)),
                          0,
                          sin(deg2rad(deviation_angle)),
                          cos(deg2rad(deviation_angle)),
                          0,
                          0,
                          0,
                          1};
    ...
    {
      Matx31f point_3d{rune_points_3d[i].x, rune_points_3d[i].y,
                       rune_points_3d[i].z};             // 靶心坐标系下的点
      ...
      rotated_points_3d[i] =
          Point3f(rotated_point_3d(0), rotated_point_3d(1), rotated_point_3d(2));
    }
    // 设置点集
    group_points_2d.insert(group_points_2d.end(), rune_points_2d.begin(),
                           rune_points_2d.end());
    group_points_3d.insert(group_points_3d.end(), rotated_points_3d.begin(),
                           rotated_points_3d.end());
    group_points_weight.insert(group_points_weight.end(), rune_points_weight.begin(),
                               rune_points_weight.end());
  }

  if (!(group_points_2d.size() == group_points_3d.size() &&
        group_points_2d.size() == group_points_weight.size()))
  {
...
inline bool correctPnpData(const FeatureNode_ptr& group, const PoseNode& src,
                           PoseNode& dst)
{
  dst = src;
  return true;
...
inline int getValidPointCount(const vector<Point2f>& points_2d)
{
  ...
  for (auto& pt : points_2d)
  {
    if (std::none_of(unique_points.begin(), unique_points.end(),
                     [&](const Point2f& p) { return norm(p - pt) < 1e-5; }))
    {
...
{
  if (!group || points_2d.size() < 4 || points_3d.size() != points_2d.size() ||
      points_2d.size() != point_weights.size())
    return false;
  return getValidPointCount(points_2d) >= 4;
}

bool RuneDetector::getCameraPnpData(const FeatureNode_ptr& group,
                                    ...
                                    PoseNode& pnp_data) const
{
  ...
  Matx31f rvec, tvec;
  solvePnP(points_3d, points_2d, camera_param.cameraMatrix, camera_param.distCoeff, rvec,
           tvec, false, SOLVEPNP_SQPNP);
...
bool RuneDetector::getPnpData(PoseNode& camera_pnp_data, const FeatureNode_ptr& group,
                              const vector<RuneFeatureComboConst>& matched_features) const
{
  if (!group || matched_features.empty()) return false;

  vector<Point2f> points_2d;
  vector<Point3f> points_3d;
  vector<float> point_weights;
  if (!getPnpPoints(group, matched_features, points_2d, points_3d, point_weights) ||
      points_2d.size() < 4)
    return false;
