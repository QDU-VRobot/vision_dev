...
void process(cv::VideoCapture& vid_cap)
{
  ...
  input.setColor(rune_detect_demo_param.color_type == 0 ? PixChannel::RED
                                                        : PixChannel::BLUE);
  ...
  if (rune_groups.empty()) return;
  auto rune_group = RuneGroup::cast(rune_groups.front());
  if (rune_group->childFeatures().empty()) return;
  ...
  for (auto tracker : rune_group->getTrackers())
  {
    ...
    if (tracker_->getHistoryNodes().size() < 2) continue;
...
void updateParam(cv::VideoCapture& cap)
{
  ...
  do
  {
    if (rune_detect_demo_param.is_get_total_frames == false) break;
    if (layout->hasWindow(DebugTools::get()->getWindowName()))
    {
      ...
      if (count % 50 == 0)  // 每30帧更新一次
      {
        ...
        setTrackbarPos("Frame Processed", DebugTools::get()->getWindowName(),
                       current_frame);
      }
    }
    else
