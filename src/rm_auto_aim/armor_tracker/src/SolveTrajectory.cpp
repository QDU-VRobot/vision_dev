...
float SolveTrajectory::SolvePitch(float x, float y, float z)
{
  ...
  if (calculate_mode_ == CalculateMode::TABLE_LOOKUP && table_->IsInit())
  {
    ...
    RCLCPP_DEBUG(logger_, "Table lookup - s: %.2f, z: %.2f, pitch: %.4f", target_s,
                 target_z, pitch);
  }
  else
  {
...
bool SolveTrajectory::CanFire(float aim_yaw, float max_yaw_diff,
                              const auto_aim_interfaces::msg::Target::SharedPtr& msg)
{
  RCLCPP_DEBUG(logger_, "x_v_diff=%.3f, y_v_diff=%.3f,yaw_diff=%.3f,cam_to_x=%.3f",
...
int SolveTrajectory::SelectArmor(const auto_aim_interfaces::msg::Target::SharedPtr& msg)
{
  ...
  {
    float aim_yaw =
        fabsf(pre_position_[i].yaw - SolveYaw(pre_position_[i].x, pre_position_[i].y));
    if (aim_yaw < min_yaw)
    {
      min_yaw = aim_yaw;
      selected_idx = i;
    ...
    const auto_aim_interfaces::msg::Target::SharedPtr& msg)
{
  ...
  if (last_selected_idx_ == LOST)
  {
    ...
    RCLCPP_DEBUG(logger_, "fly_time=%.3fs, turn_time= %.3fs, selected_idx=%d", fly_time_,
                 turn_time, selected_idx);
  }
  else
  {
    int selected_idx = SelectArmor(msg);
